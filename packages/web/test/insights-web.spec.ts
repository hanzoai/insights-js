...
import { waitForPromises } from '@hanzo/insights-core/testing'
import { Insights } from '../src'

describe('InsightsWeb', () => {
  let fetch: jest.Mock
  jest.useRealTimers()
  ...
  describe('init', () => {
    ...
    it('should load feature flags on init', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('https://us.i.insights.com/flags/?v=2', {
        body: JSON.stringify({
          token: 'TEST_API_KEY',
  ...
  describe('History API tracking', () => {
    ...
    it('should include navigation properties in capture call and rely on getCommonEventProperties', async () => {
      ...
      const commonEventProps = { $lib: '@hanzo/insights-lite', $lib_version: '1.0.0' }
      const getCommonEventPropertiesSpy = jest
        .spyOn(insights, 'getCommonEventProperties')
        .mockImplementation(() => commonEventProps)

      const coreCaptureMethod = jest.spyOn(Insights.prototype, 'capture')
