...
describe('Insights Core', () => {
  ...
  describe('capture', () => {
    it('should capture an event', async () => {
      ...
      expect(url).toMatch(/^https:\/\/us\.i\.insights\.com\/batch\//)
    ...
    it('should allow overriding the timestamp', async () => {
      ...
      insights.capture('custom-event', { foo: 'bar' }, { timestamp: new Date('2021-01-02') })
      await waitForPromises()
      const body = parseBody(mocks.fetch.mock.calls[0])
    ...
    it('should allow overriding the uuid', async () => {
      ...
      insights.capture('custom-event', { foo: 'bar' }, { uuid: id })
      await waitForPromises()
      const body = parseBody(mocks.fetch.mock.calls[0])
  ...
  describe('before_send', () => {
    it('should allow dropping events by returning null', async () => {
      ...
      insights.capture('custom-event', { foo: 'bar' })
      await waitForPromises()
    ...
    it('should allow modifying events', async () => {
      ...
      insights.capture('original-event', { original: 'value' })
      await waitForPromises()
    ...
    it('should pass timestamp and uuid through before_send', async () => {
      ...
      insights.capture('custom-event', {}, { timestamp: customDate, uuid: customUuid })
      await waitForPromises()
    ...
    it('should expose $set and $set_once from identify events', async () => {
      ...
      insights.identify('user-123', { $set: { name: 'John' }, $set_once: { created_at: '2023-01-01' } })
      await waitForPromises()
    ...
    it('should allow modifying $set in before_send for identify events', async () => {
      ...
      insights.identify('user-123', { $set: { name: 'John' } })
      await waitForPromises()
