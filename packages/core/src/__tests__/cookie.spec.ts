...
describe('getInsightsCookieName', () => {
  it.each([
    ['simple API key', 'phc_abc123', 'hi_phc_abc123_insights'],
    ['sanitizes + in token', 'abc+def', 'hi_abcPLdef_insights'],
    ['sanitizes / in token', 'abc/def', 'hi_abcSLdef_insights'],
    ['sanitizes = in token', 'abc=def', 'hi_abcEQdef_insights'],
    ['sanitizes multiple special characters', 'a+b/c=d', 'hi_aPLbSLcEQd_insights'],
...
describe('readInsightsCookie', () => {
  it('reads and parses a Insights cookie via the store', () => {
    ...
    const store = cookieStoreFromHeader(`hi_phc_test_insights=${encodeURIComponent(cookieValue)}`)
