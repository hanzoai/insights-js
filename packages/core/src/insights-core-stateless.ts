...
class InsightsFetchHttpError extends Error {
  name = 'InsightsFetchHttpError'

  constructor(
    public response: InsightsFetchResponse,
    public reqByteLength: number
  ) {
    super('HTTP error while fetching Insights: status=' + response.status + ', reqByteLength=' + reqByteLength)
  }
...
class InsightsFetchNetworkError extends Error {
  name = 'InsightsFetchNetworkError'

  constructor(public error: unknown) {
    // TRICKY: "cause" is a newer property but is just ignored otherwise. Cast to any to ignore the type issue.
    // eslint-disable-next-line @typescript-eslint/prefer-ts-expect-error
    // @ts-ignore
    super('Network error while fetching Insights', error instanceof Error ? { cause: error } : {})
  }
}
...
export async function logFlushError(err: any): Promise<void> {
  if (err instanceof InsightsFetchHttpError) {
    let text = ''
    try {
      text = await err.text
    } catch {}

    console.error(`Error while flushing Insights: message=${err.message}, response body=${text}`, err)
  } else {
    console.error('Error while flushing Insights', err)
  }
  return Promise.resolve()
}

function isInsightsFetchError(err: unknown): err is InsightsFetchHttpError | InsightsFetchNetworkError {
...
export abstract class InsightsCoreStateless {
  // options
  ...
  protected _remoteConfigResponsePromise?: Promise<InsightsRemoteConfig | undefined>
  protected _logger: Logger

  // Abstract methods to be overridden by implementations
  abstract fetch(url: string, options: InsightsFetchOptions): Promise<InsightsFetchResponse>
  abstract getLibraryId(): string
  abstract getLibraryVersion(): string
  abstract getCustomUserAgent(): string | void

  // This is our abstracted storage. Each implementation should handle its own
  abstract getPersistedProperty<T>(key: InsightsPersistedProperty): T | undefined
  ...
  constructor(apiKey: string, options: InsightsCoreOptions = {}) {
    ...
    this._retryOptions = {
      ...
      retryCheck: isInsightsFetchError,
    }
  ...
  public get optedOut(): boolean {
    return this.getPersistedProperty(InsightsPersistedProperty.OptedOut) ?? !this.defaultOptIn
  }

  async optIn(): Promise<void> {
    this.wrap(() => {
      this.setPersistedProperty(InsightsPersistedProperty.OptedOut, false)
    })
  }

  async optOut(): Promise<void> {
    this.wrap(() => {
      this.setPersistedProperty(InsightsPersistedProperty.OptedOut, true)
    })
  }
   ...
   *
   * @remarks
   * Debug mode logs all Insights calls to the console for troubleshooting.
  ...
  protected groupIdentifyStateless(
    ...
    options?: InsightsCaptureOptions,
    distinctId?: string,
    eventProperties?: InsightsEventProperties
  ): void {
    this.wrap(() => {
      const payload = this.buildPayload({
  ...
  protected async getRemoteConfig(): Promise<InsightsRemoteConfig | undefined> {
    ...
    return this.fetchWithRetry(url, fetchOptions, { retryCount: 0 }, this.remoteConfigRequestTimeoutMs)
      .then((response) => response.json() as Promise<InsightsRemoteConfig>)
      .catch((error) => {
  ...
  protected async getFlags(
    ...
    return this.fetchWithRetry(url, fetchOptions, { retryCount: 0 }, this.featureFlagsRequestTimeoutMs)
      .then((response) => response.json() as Promise<InsightsV1FlagsResponse | InsightsV2FlagsResponse>)
  ...
  private categorizeRequestError(error: unknown): FeatureFlagRequestError {
    if (error instanceof InsightsFetchHttpError) {
      return { type: 'api_error', statusCode: error.status }
    }

    if (error instanceof InsightsFetchNetworkError) {
  ...
  ): Promise<InsightsFlagsResponse['featureFlagPayloads'] | undefined> {
    await this._initPromise
  ...
  protected async getFeatureFlagsStateless(
    ...
    requestId: InsightsFlagsResponse['requestId'] | undefined
  }> {
    await this._initPromise
  ...
  protected async getFeatureFlagDetailsStateless(
    ...
    disableGeoip?: boolean,
    flagKeysToEvaluate?: string[]
  ): Promise<InsightsFeatureFlagDetails | undefined> {
    await this._initPromise
   ...
   *** SUPER PROPERTIES
   ***/
  private _props: InsightsEventProperties | undefined

  protected get props(): InsightsEventProperties {
    if (!this._props) {
      this._props = this.getPersistedProperty<InsightsEventProperties>(InsightsPersistedProperty.Props)
    }
    return this._props || {}
  }

  protected set props(val: InsightsEventProperties | undefined) {
    this._props = val
  }

  async register(properties: InsightsEventProperties): Promise<void> {
    this.wrap(() => {
      this.props = {
        ...this.props,
        ...properties,
      }
      this.setPersistedProperty<InsightsEventProperties>(InsightsPersistedProperty.Props, this.props)
    })
  }
  ...
  protected enqueue(type: string, _message: any, options?: InsightsCaptureOptions): void {
    this.wrap(() => {
      if (this.optedOut) {
        this._events.emit(type, `Library is disabled. Not sending event. To re-enable, call insights.optIn()`)
        return
      }

      let message: InsightsEventProperties | null = this.prepareMessage(type, _message, options)
      ...
      const queue = this.getPersistedProperty<InsightsQueueItem[]>(InsightsPersistedProperty.Queue) || []
      ...
      queue.push({ message })
      this.setPersistedProperty<InsightsQueueItem[]>(InsightsPersistedProperty.Queue, queue)
  ...
  protected async sendImmediate(type: string, _message: any, options?: InsightsCaptureOptions): Promise<void> {
    if (this.disabled) {
      this._logger.warn('The client is disabled')
      return
    }
  ...
  protected prepareMessage(type: string, _message: any, options?: InsightsCaptureOptions): InsightsEventProperties {
    const message = {
      ..._message,
      type: type,
   ...
   * @public
   *
   * @throws InsightsFetchHttpError
   * @throws InsightsFetchNetworkError
   * @throws Error
   */
  ...
  private async _flush(): Promise<void> {
    ...
    let queue = this.getPersistedProperty<InsightsQueueItem[]>(InsightsPersistedProperty.Queue) || []
    ...
    while (queue.length > 0 && sentMessages.length < originalQueueLength) {
      ...
      const persistQueueChange = async (): Promise<void> => {
        const refreshedQueue = this.getPersistedProperty<InsightsQueueItem[]>(InsightsPersistedProperty.Queue) || []
        const newQueue = refreshedQueue.slice(batchItems.length)
        this.setPersistedProperty<InsightsQueueItem[]>(InsightsPersistedProperty.Queue, newQueue)
        queue = newQueue
    ...
    return await retriable(
      async () => {
        ...
        if (!isNoCors && (res.status < 200 || res.status >= 400)) {
          throw new InsightsFetchHttpError(res, reqByteLength)
        }
        return res
      },
  ...
  async _shutdown(shutdownTimeoutMs: number = 30000): Promise<void> {
    ...
    const doShutdown = async (): Promise<void> => {
      try {
        ...
        while (true) {
          ...
          // For example, see sendFeatureFlags in insights-node/src/insights-node.ts::capture
          await this.flush()
    ...
    return Promise.race([
      new Promise<void>((_, reject) => {
        safeSetTimeout(() => {
          this._logger.error('Timed out while shutting down Insights')
          hasTimedOut = true
          reject('Timeout while shutting down Insights. Some events may not have been sent.')
        }, shutdownTimeoutMs)
      }),
      doShutdown(),
    ])
  ...
  /**
   * Shuts down the Insights instance and ensures all events are sent.
   *
   * Call shutdown() once before the process exits to ensure that all events have been sent and all promises
   * have resolved. Do not use this function if you intend to keep using this Insights instance after calling it.
