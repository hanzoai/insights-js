import type {
  InsightsAutocaptureElement,
  InsightsFlagsResponse,
  InsightsFeatureFlagsResponse,
  InsightsCoreOptions,
  ...
  InsightsV2FlagsResponse,
  InsightsV1FlagsResponse,
  InsightsFeatureFlagDetails,
  InsightsFlagsStorageFormat,
  FeatureFlagDetail,
  Survey,
  SurveyResponse,
  InsightsGroupProperties,
  BeforeSendFn,
  CaptureEvent,
} from './types'
import {
...
interface PendingFlagsRequest extends FlagsAsyncOptions {
  resolve: (value: InsightsFeatureFlagsResponse | undefined) => void
  reject: (reason?: unknown) => void
}

export abstract class InsightsCore extends InsightsCoreStateless {
  // options
  ...
  // internal
  protected _flagsResponsePromise?: Promise<InsightsFeatureFlagsResponse | undefined>
  protected _sessionExpirationTimeSeconds: number
  private _sessionMaxLengthSeconds: number = 24 * 60 * 60 // 24 hours
  protected sessionProps: InsightsEventProperties = {}
  ...
  protected setupBootstrap(options?: Partial<InsightsCoreOptions>): void {
    ...
    if (bootstrap.distinctId) {
      if (bootstrap.isIdentifiedId) {
        const distinctId = this.getPersistedProperty(InsightsPersistedProperty.DistinctId)

        if (!distinctId) {
          this.setPersistedProperty(InsightsPersistedProperty.DistinctId, bootstrap.distinctId)
          // Mark the user as identified if bootstrapping with an identified ID
          this.setPersistedProperty(InsightsPersistedProperty.PersonMode, 'identified')
        }
      } else {
        const anonymousId = this.getPersistedProperty(InsightsPersistedProperty.AnonymousId)

        if (!anonymousId) {
          this.setPersistedProperty(InsightsPersistedProperty.AnonymousId, bootstrap.distinctId)
        }
      }
    }
  ...
  reset(propertiesToKeep?: InsightsPersistedProperty[]): void {
    this.wrap(() => {
      const allPropertiesToKeep = [InsightsPersistedProperty.Queue, ...(propertiesToKeep || [])]
      ...
      for (const key of <(keyof typeof InsightsPersistedProperty)[]>Object.keys(InsightsPersistedProperty)) {
        if (!allPropertiesToKeep.includes(InsightsPersistedProperty[key])) {
          this.setPersistedProperty((InsightsPersistedProperty as any)[key], null)
        }
      }
  ...
  protected getCommonEventProperties(): InsightsEventProperties {
  ...
  private enrichProperties(properties?: InsightsEventProperties): InsightsEventProperties {
    return {
  ...
  getSessionId(): string {
    ...
    let sessionId = this.getPersistedProperty<string>(InsightsPersistedProperty.SessionId)
    const sessionLastTimestamp = this.getPersistedProperty<number>(InsightsPersistedProperty.SessionLastTimestamp) || 0
    const sessionStartTimestamp = this.getPersistedProperty<number>(InsightsPersistedProperty.SessionStartTimestamp) || 0
    const now = Date.now()
    ...
    if (
      ...
      sessionId = uuidv7()
      this.setPersistedProperty(InsightsPersistedProperty.SessionId, sessionId)
      this.setPersistedProperty(InsightsPersistedProperty.SessionStartTimestamp, now)
    }
    this.setPersistedProperty(InsightsPersistedProperty.SessionLastTimestamp, now)
  ...
  resetSessionId(): void {
    this.wrap(() => {
      this.setPersistedProperty(InsightsPersistedProperty.SessionId, null)
      this.setPersistedProperty(InsightsPersistedProperty.SessionLastTimestamp, null)
      this.setPersistedProperty(InsightsPersistedProperty.SessionStartTimestamp, null)
    })
  }
   ...
   * const anonId = insights.getAnonymousId()
  ...
  getAnonymousId(): string {
    ...
    let anonId = this.getPersistedProperty<string>(InsightsPersistedProperty.AnonymousId)
    if (!anonId) {
      anonId = uuidv7()
      this.setPersistedProperty(InsightsPersistedProperty.AnonymousId, anonId)
    }
    return anonId
  }
  ...
  getDistinctId(): string {
    ...
    return this.getPersistedProperty<string>(InsightsPersistedProperty.DistinctId) || this.getAnonymousId()
  }

  registerForSession(properties: InsightsEventProperties): void {
    this.sessionProps = {
      ...this.sessionProps,
      ...properties,
    }
  ...
  identify(distinctId?: string, properties?: InsightsEventProperties, options?: InsightsCaptureOptions): void {
    this.wrap(() => {
      if (!this._requirePersonProcessing('insights.identify')) {
        return
      }
      ...
      if (properties?.$groups) {
        this.groups(properties.$groups as InsightsGroupProperties)
      }
      ...
      if (distinctId !== previousDistinctId) {
        ...
        this.setPersistedProperty(InsightsPersistedProperty.AnonymousId, previousDistinctId)
        this.setPersistedProperty(InsightsPersistedProperty.DistinctId, distinctId)
  ...
  capture(event: string, properties?: InsightsEventProperties, options?: InsightsCaptureOptions): void {
  ...
  alias(alias: string): void {
    this.wrap(() => {
      if (!this._requirePersonProcessing('insights.alias')) {
        return
      }
  ...
  autocapture(
    eventType: string,
    elements: InsightsAutocaptureElement[],
  ...
  groups(groups: InsightsGroupProperties): void {
    this.wrap(() => {
      if (!this._requirePersonProcessing('insights.group')) {
        return
      }
      ...
      this.register({
        $groups: {
          ...(existingGroups as InsightsGroupProperties),
          ...groups,
        },
      })
  ...
  group(
    ...
    groupProperties?: InsightsEventProperties,
    options?: InsightsCaptureOptions
  ): void {
    this.wrap(() => {
      this.groups({
  ...
  groupIdentify(
    ...
    this.wrap(() => {
      if (!this._requirePersonProcessing('insights.groupIdentify')) {
        return
      }
  ...
  resetPersonPropertiesForFlags(): void {
    this.wrap(() => {
      this.setPersistedProperty<InsightsEventProperties>(InsightsPersistedProperty.PersonProperties, null)
    })
  }
  ...
  setGroupPropertiesForFlags(properties: { [type: string]: Record<string, string> }): void {
    this.wrap(() => {
      ...
      this.setPersistedProperty<InsightsEventProperties>(InsightsPersistedProperty.GroupProperties, {
        ...existingProperties,
        ...properties,
      })
    })
  ...
  resetGroupPropertiesForFlags(): void {
    this.wrap(() => {
      this.setPersistedProperty<InsightsEventProperties>(InsightsPersistedProperty.GroupProperties, null)
    })
  }
  ...
  protected onRemoteConfig(_response: InsightsRemoteConfig): void {
   ...
   */
  private maybeNotifyRemoteConfig(shouldTrigger: boolean, response: InsightsRemoteConfig): void {
    if (shouldTrigger) {
      try {
  ...
  private cacheSessionReplay(source: string, response?: InsightsRemoteConfig): void {
    const sessionReplay = response?.sessionRecording
    if (sessionReplay) {
      this.setPersistedProperty(InsightsPersistedProperty.SessionReplay, sessionReplay)
      ...
      this.setPersistedProperty(InsightsPersistedProperty.SessionReplay, null)
    }
  }

  private async _remoteConfigAsync(): Promise<InsightsRemoteConfig | undefined> {
    this._remoteConfigResponsePromise = this._initPromise
      .then(() => {
        let remoteConfig = this.getPersistedProperty<Omit<InsightsRemoteConfig, 'surveys'>>(
          InsightsPersistedProperty.RemoteConfig
        )
            ...
            if (this.disableSurveys === false) {
              ...
              if (hasSurveys) {
                this.setPersistedProperty<SurveyResponse['surveys']>(
                  InsightsPersistedProperty.Surveys,
                  surveys as Survey[]
                )
              } else {
                this.setPersistedProperty<SurveyResponse['surveys']>(InsightsPersistedProperty.Surveys, null)
              }
            } else {
              this.setPersistedProperty<SurveyResponse['surveys']>(InsightsPersistedProperty.Surveys, null)
            }
            // we cache the surveys in its own storage key
            this.setPersistedProperty<Omit<InsightsRemoteConfig, 'surveys'>>(
              InsightsPersistedProperty.RemoteConfig,
  ...
  private async _flagsAsync(options?: FlagsAsyncOptions): Promise<InsightsFeatureFlagsResponse | undefined> {
    ...
    this._flagsResponsePromise = this._initPromise
      .then(async () => {
        ...
        const personProperties =
          this.getPersistedProperty<Record<string, string>>(InsightsPersistedProperty.PersonProperties) || {}
        ...
        const result = await super.getFlags(
          distinctId,
          groups as InsightsGroupProperties,
          personProperties,
          groupProperties,
          extraProperties,
          fetchConfig
        ...
        if (res?.featureFlags) {
          ...
          this.setPersistedProperty(InsightsPersistedProperty.FlagsEndpointWasHit, true)
  ...
  private setKnownFeatureFlagDetails(flagsResponse: InsightsFlagsStorageFormat | null): void {
    this.wrap(() => {
      this.setPersistedProperty<InsightsFlagsStorageFormat>(InsightsPersistedProperty.FeatureFlagDetails, flagsResponse)

      this._events.emit('featureflags', getFlagValuesFromFlags(flagsResponse?.flags ?? {}))
    })
  }

  private getKnownFeatureFlagDetails(): InsightsFeatureFlagDetails | undefined {
    const storedDetails = this.getPersistedProperty<InsightsFlagsStorageFormat>(
      InsightsPersistedProperty.FeatureFlagDetails
    )
    if (!storedDetails) {
      // Rebuild from the stored feature flags and feature flag payloads
      const featureFlags = this.getPersistedProperty<InsightsFlagsResponse['featureFlags']>(
        InsightsPersistedProperty.FeatureFlags
      )
      const featureFlagPayloads = this.getPersistedProperty<InsightsFlagsResponse['featureFlagPayloads']>(
        InsightsPersistedProperty.FeatureFlagPayloads
      )
    ...
    return normalizeFlagsResponse(
      storedDetails as InsightsV1FlagsResponse | InsightsV2FlagsResponse
    ) as InsightsFeatureFlagDetails
  }

  private getStoredFlagDetails(): InsightsFlagsStorageFormat | undefined {
    return this.getPersistedProperty<InsightsFlagsStorageFormat>(InsightsPersistedProperty.FeatureFlagDetails)
  }

  protected getKnownFeatureFlags(): InsightsFlagsResponse['featureFlags'] | undefined {
  ...
  private getBootstrappedFeatureFlagDetails(): InsightsFeatureFlagDetails | undefined {
    const details = this.getPersistedProperty<InsightsFeatureFlagDetails>(
      InsightsPersistedProperty.BootstrapFeatureFlagDetails
    )
    if (!details) {
      return undefined
    }
  ...
  private setBootstrappedFeatureFlagDetails(details: InsightsFeatureFlagDetails): void {
    this.setPersistedProperty<InsightsFeatureFlagDetails>(InsightsPersistedProperty.BootstrapFeatureFlagDetails, details)
  }

  private getBootstrappedFeatureFlags(): InsightsFlagsResponse['featureFlags'] | undefined {
  ...
  private getBootstrappedFeatureFlagPayloads(): InsightsFlagsResponse['featureFlagPayloads'] | undefined {
    const details = this.getBootstrappedFeatureFlagDetails()
    if (!details) {
      return undefined
    }
  ...
  protected _getFeatureFlagResult(
    ...
    if (sendEvent) {
      ...
      const properties: Record<string, any> = {
        ...
        $used_bootstrap_value: !this.getPersistedProperty(InsightsPersistedProperty.FlagsEndpointWasHit),
  ...
  getFeatureFlagPayloads(): InsightsFlagsResponse['featureFlagPayloads'] | undefined {
    return this.getFeatureFlagDetails()?.featureFlagPayloads
  }

  getFeatureFlags(): InsightsFlagsResponse['featureFlags'] | undefined {
  ...
  getFeatureFlagDetails(): InsightsFeatureFlagDetails | undefined {
    // NOTE: We don't check for _initPromise here as the function is designed to be
    // callable before the state being loaded anyways
    let details = this.getKnownFeatureFlagDetails()
    const overriddenFlags = this.getPersistedProperty<InsightsFlagsResponse['featureFlags']>(
    ...
    return normalizeFlagsResponse(result) as InsightsFeatureFlagDetails
  }

  getFeatureFlagsAndPayloads(): {
    flags: InsightsFlagsResponse['featureFlags'] | undefined
    payloads: InsightsFlagsResponse['featureFlagPayloads'] | undefined
  } {
  ...
  reloadFeatureFlags(options?: { cb?: (err?: Error, flags?: InsightsFlagsResponse['featureFlags']) => void }): void {
  ...
  async reloadRemoteConfigAsync(): Promise<InsightsRemoteConfig | undefined> {
  ...
  onFeatureFlags(cb: (flags: InsightsFlagsResponse['featureFlags']) => void): () => void {
  ...
  async overrideFeatureFlag(flags: InsightsFlagsResponse['featureFlags'] | null): Promise<void> {
    this.wrap(() => {
      if (flags === null) {
        return this.setPersistedProperty(InsightsPersistedProperty.OverrideFeatureFlags, null)
      }
      return this.setPersistedProperty(InsightsPersistedProperty.OverrideFeatureFlags, flags)
    })
  }
  ...
  protected _isIdentified(): boolean {
    const personMode = this.getPersistedProperty<string>(InsightsPersistedProperty.PersonMode)
    ...
    if (personMode === undefined) {
      const distinctId = this.getPersistedProperty<string>(InsightsPersistedProperty.DistinctId)
      const anonymousId = this.getPersistedProperty<string>(InsightsPersistedProperty.AnonymousId)
   ...
   * @internal
   */
  protected _getGroups(): InsightsGroupProperties {
    return (this.props.$groups || {}) as InsightsGroupProperties
  }
  ...
  protected _hasPersonProcessing(): boolean {
    ...
    const personProcessingEnabled =
      this.getPersistedProperty<boolean>(InsightsPersistedProperty.EnablePersonProcessing) === true
  ...
  protected _requirePersonProcessing(functionName: string): boolean {
    ...
    this.setPersistedProperty(InsightsPersistedProperty.EnablePersonProcessing, true)
    return true
  }
  ...
  createPersonProfile(): void {
    ...
    if (!this._requirePersonProcessing('insights.createPersonProfile')) {
      return
    }
   ...
   *
   * @remarks
   * Updates user properties that are stored with the person profile in Insights.
  ...
  setPersonProperties(
    ...
    this.wrap(() => {
      ...
      if (!this._requirePersonProcessing('insights.setPersonProperties')) {
        return
      }
   ...
   */
  protected processBeforeEnqueue(message: InsightsEventProperties): InsightsEventProperties | null {
    if (!this._beforeSend) {
      return message
    }
    ...
    const props = (message.properties || {}) as InsightsEventProperties
    const captureEvent: CaptureEvent = {
      uuid: message.uuid as string,
      event: message.event as string,
      properties: props,
      $set: props.$set as InsightsEventProperties | undefined,
      $set_once: props.$set_once as InsightsEventProperties | undefined,
    ...
    const resultProps = { ...(result.properties ?? props) } as InsightsEventProperties
