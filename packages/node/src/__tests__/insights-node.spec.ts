import { Insights, InsightsOptions } from '@/entrypoints/index.node'
...
describe('Insights Node.js', () => {
  ...
  describe('core methods', () => {
    it('should capture an event to shared queue', async () => {
      ...
      insights.capture({ distinctId: '123', event: 'test-event', properties: { foo: 'bar' }, groups: { org: 123 } })
    ...
    it('should capture identify events on shared queue', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      insights.identify({ distinctId: '123', properties: { foo: 'bar' } })
      jest.runOnlyPendingTimers()
      await waitForPromises()
    ...
    it('should handle identify using $set and $set_once', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      insights.identify({ distinctId: '123', properties: { $set: { foo: 'bar' }, $set_once: { vip: true } } })
      jest.runOnlyPendingTimers()
      await waitForPromises()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
    ...
    it('should handle identify using $set_once', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      insights.identify({ distinctId: '123', properties: { foo: 'bar', $set_once: { vip: true } } })
      jest.runOnlyPendingTimers()
      await waitForPromises()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
    ...
    it('should capture alias events on shared queue', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      insights.alias({ distinctId: '123', alias: '1234' })
      jest.runOnlyPendingTimers()
      await waitForPromises()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
    ...
    it('should allow overriding uuid', async () => {
      ...
      insights.capture({ event: 'custom-time', distinctId: '123', uuid })
      await waitForFlushTimer()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
        {
    ...
    it('should use default is set, and override on specific disableGeoip calls', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      const client = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
        disableGeoip: false,
        disableCompression: true,
      })
    ...
    it('should include registered properties in captured events', async () => {
      insights.register({ registered_prop: 'registered_value', another: 123 })

      insights.capture({ distinctId: '123', event: 'test-event', properties: { foo: 'bar' } })
    ...
    it('should allow capture properties to override registered properties', async () => {
      insights.register({ foo: 'registered_value', only_registered: 'yes' })

      insights.capture({ distinctId: '123', event: 'test-event', properties: { foo: 'override_value' } })
    ...
    it('should stop including unregistered properties', async () => {
      insights.register({ foo: 'bar', remove_me: 'value' })
      insights.unregister('remove_me')

      insights.capture({ distinctId: '123', event: 'test-event' })
  ...
  describe('before_send', () => {
    ...
    it('should log when event is dropped in debug mode', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith('[Insights]', "Event 'test-event' was rejected in beforeSend function")
      logSpy.mockRestore()
    })
  })
  ...
  describe('feature flags', () => {
    beforeEach(() => {
      ...
      const multivariateFlag = {
        ...
        filters: {
          groups: [
            {
              properties: [{ key: 'email', type: 'person', value: 'test@insights.com', operator: 'exact' }],
              rollout_percentage: 100,
            },
            {
    ...
    it('should do getFeatureFlag', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(insights.getFeatureFlag('feature-variant', '123', { groups: { org: '123' } })).resolves.toEqual(
        'variant'
      )
    ...
    it('should do isFeatureEnabled', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(insights.isFeatureEnabled('feature-1', '123', { groups: { org: '123' } })).resolves.toEqual(true)
      await expect(insights.isFeatureEnabled('feature-4', '123', { groups: { org: '123' } })).resolves.toEqual(undefined)
    ...
    it('should use minimum featureFlagsPollingInterval of 100ms if set less to less than 100', async () => {
      ...
      expect(insights.options.featureFlagsPollingInterval).toEqual(100)
    })
      ...
      it('should work with explicit person properties and preserve event properties', async () => {
        ...
        insights.capture({
          ...
          properties: {
            ...
            $lib: 'insights-node', // Should be passed through
            numericValue: 123,
            booleanValue: true,
          },
        })
    ...
    it('manages memory well when sending feature flags', async () => {
      ...
      expect(Object.keys(insights.distinctIdHasSentFlagCalls).length).toEqual(0)

      for (let i = 0; i < 100; i++) {
        const distinctId = `some-distinct-id${i}`
        await insights.getFeatureFlag('beta-feature', distinctId)
        ...
        expect(Object.keys(insights.distinctIdHasSentFlagCalls).length <= 10).toEqual(true)
      }
    })
    ...
    it('$feature_flag_called is called appropriately when querying flags', async () => {
      ...
      expect(
        await insights.getFeatureFlag('beta-feature', 'some-distinct-id2', {
          groups: { x: 'y' },
          personProperties: { region: 'USA', name: 'Aloha' },
          disableGeoip: false,
        })
      ...
      expect(
        await insights.getFeatureFlag('beta-feature', 'some-distinct-id23', {
          personProperties: { region: 'USA', name: 'Aloha' },
      ...
      expect(
        await insights.getFeatureFlag('flags-flag', 'some-distinct-id2345', {
      ...
      expect(
        await insights.isFeatureEnabled('flags-flag', 'some-distinct-id2345', {
          groups: { organization: 'org1' },
          personProperties: { region: 'USA', name: 'Aloha' },
        })
      ).toEqual(true)
    ...
    describe('`sendFeatureFlagEvent` client option', () => {
      ...
      describe.each(methods)('$name', ({ name: methodName, expectedValue }) => {
        it('respects client sendFeatureFlagEvent option when method-level option is not provided', async () => {
          ...
          const result = await insights[methodName]('beta-feature', 'some-distinct-id')
        ...
        it('overrides client sendFeatureFlagEvent option when method-level option is provided', async () => {
          ...
          const result = await insights[methodName]('beta-feature', 'some-distinct-id', { sendFeatureFlagEvents: true })
          expect(result).toEqual(expectedValue)
    ...
    it('should do getFeatureFlagPayloads', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(
        insights.getFeatureFlagPayload('feature-variant', '123', 'variant', { groups: { org: '123' } })
      ).resolves.toEqual(2)
      expect(mockedFetch).toHaveBeenCalledTimes(1)
      expect(mockedFetch).toHaveBeenCalledWith(
        'http://example.com/flags/?v=2&config=true',
    ...
    it('should not double parse json with getFeatureFlagPayloads and local eval', async () => {
      ...
      await expect(
        insights.getFeatureFlagPayload('feature-array', '123', true, { onlyEvaluateLocally: true })
      ).resolves.toEqual([1])
      expect(mockedFetch).toHaveBeenCalledTimes(0)

      await expect(insights.getFeatureFlagPayload('feature-array', '123')).resolves.toEqual([1])
      expect(mockedFetch).toHaveBeenCalledTimes(0)

      await expect(insights.getFeatureFlagPayload('false-flag', '123', true)).resolves.toEqual(300)
    ...
    it('should not double parse json with getFeatureFlagPayloads and server eval', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(
        insights.getFeatureFlagPayload('feature-array', '123', undefined, { groups: { org: '123' } })
      ).resolves.toEqual([1])
      expect(mockedFetch).toHaveBeenCalledTimes(1)
      expect(mockedFetch).toHaveBeenCalledWith(
        'http://example.com/flags/?v=2&config=true',
    ...
    it('should do getFeatureFlagPayloads without matchValue', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(
        insights.getFeatureFlagPayload('feature-variant', '123', undefined, { groups: { org: '123' } })
      ).resolves.toEqual(2)
      expect(mockedFetch).toHaveBeenCalledTimes(1)
    })
    ...
    it('should add default person & group properties for feature flags', async () => {
      ...
      // test nones
      await insights.getAllFlagsAndPayloads('some_id', {
        groups: undefined,
      ...
      mockedFetch.mockClear()
      await insights.getAllFlags('some_id', {
        groups: { company: 'id:5' },
        personProperties: undefined,
        groupProperties: undefined,
      })
      ...
      mockedFetch.mockClear()
      await insights.getFeatureFlagPayload('random_key', 'some_id', undefined)
      jest.runOnlyPendingTimers()
      ...
      await insights.isFeatureEnabled('random_key', 'some_id')
      jest.runOnlyPendingTimers()
  ...
  describe('evaluation contexts', () => {
    ...
    it('should send evaluation contexts when configured', async () => {
      ...
      const insightsWithEnvs = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
      ...
      await insightsWithEnvs.shutdown()
    })
    ...
    it('should not send evaluation contexts when not configured', async () => {
      ...
      const insightsWithoutEnvs = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
        ...insightsImmediateResolveOptions,
      })
      ...
      await insightsWithoutEnvs.shutdown()
    })
    ...
    it('should not send evaluation contexts when configured as empty array', async () => {
      ...
      const insightsWithEmptyEnvs = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
        evaluationContexts: [],
        ...insightsImmediateResolveOptions,
      })
      ...
      await insightsWithEmptyEnvs.shutdown()
    })
    ...
    it('should support deprecated evaluationEnvironments field', async () => {
      ...
      const insightsWithDeprecated = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
      ...
      await insightsWithDeprecated.shutdown()
    })
  })
  ...
  describe('getRemoteConfigPayload', () => {
    ...
    beforeEach(() => {
      ...
      // Initialize insights with personalApiKey
      insights = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
        fetchRetryCount: 0,
        disableCompression: true,
      ...
      requestRemoteConfigPayloadSpy = jest.spyOn(insights as any, '_requestRemoteConfigPayload')
    })

    it('should throw error when personalApiKey is not provided', async () => {
      const insightsWithoutKey = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
        fetchRetryCount: 0,
        disableCompression: true,
      })
    ...
    it('should return empty object when no payload is available', async () => {
      ...
      const payload = await insights.getRemoteConfigPayload('test-flag')
    ...
    it('should work without local evaluation enabled', async () => {
      ...
      const insightsWithoutLocalEval = new Insights('TEST_API_KEY', {
        host: 'http://example.com',
        fetchRetryCount: 0,
        disableCompression: true,
        personalApiKey: 'TEST_PERSONAL_API_KEY',
      ...
      const spy = jest.spyOn(insightsWithoutLocalEval as any, '_requestRemoteConfigPayload')
      spy.mockResolvedValue({
        json: () => Promise.resolve({ test: 'payload' }),
      })

      const payload = await insightsWithoutLocalEval.getRemoteConfigPayload('test-flag')
      expect(payload).toEqual({ test: 'payload' })
      expect(spy).toHaveBeenCalledWith('test-flag')

      // Verify that no poller was created
      expect(insightsWithoutLocalEval['featureFlagsPoller']).toBeUndefined()
    })
    ...
    it('should include project API key in the remote config URL', async () => {
      ...
      await insights.getRemoteConfigPayload('test-flag')
