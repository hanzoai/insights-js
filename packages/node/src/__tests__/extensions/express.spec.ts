...
const createRequestContextMiddleware = (insights: Insights): any => {
  const app = { use: jest.fn() }
  setupExpressRequestContext(insights, app)
  return app.use.mock.calls[0][0]
}

const createErrorHandlerMiddleware = (insights: Insights): any => {
  const app = { use: jest.fn() }
...
describe('Express extension', () => {
  ...
  describe('request context middleware', () => {
    ...
    it('should set request context for normal captures', async () => {
      const middleware = createRequestContextMiddleware(insights)
  ...
  describe('error handler', () => {
    ...
    it('should capture exceptions with sanitized session and distinct headers', async () => {
      const handler = createErrorHandlerMiddleware(insights)
