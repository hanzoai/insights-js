...
test.describe('surveys - core display logic', () => {
    test('shows the same to user if they do not dismiss or respond to it', async ({ page, context }) => {
        ...
        await expect(page.locator('.InsightsSurvey-123').locator('.survey-question')).toHaveText(
        ...
        await page.locator('.InsightsSurvey-123').locator('.survey-form').locator('textarea').fill('Great job!')
    ...
    test('rating questions that are on the 10 scale start at 0', async ({ page, context }) => {
        ...
        await expect(page.locator('.InsightsSurvey-123').locator('.survey-form')).toBeVisible()
        await expect(page.locator('.InsightsSurvey-123').locator('.ratings-number')).toHaveCount(11)
        let i = 0
        for (const rating of await page.locator('.InsightsSurvey-123').locator('.ratings-number').all()) {
            await expect(rating).toBeVisible()
            await expect(rating).toHaveText(`${i++}`)
        }

        await page.locator('.InsightsSurvey-123').locator('.ratings-number').first().click()
        await page.locator('.InsightsSurvey-123').locator('.form-submit').click()

        // After submitting, we're on question 2 (scale 5), which starts at 1
        await expect(page.locator('.InsightsSurvey-123').locator('.ratings-number').first()).toHaveText('1')
    })
    ...
    test('multiple question surveys', async ({ page, context }) => {
        ...
        await expect(page.locator('.InsightsSurvey-12345').locator('.survey-form')).toBeVisible()
        await page.locator('.InsightsSurvey-12345').locator('#surveyQuestion0Choice1').click()
        await page.locator('.InsightsSurvey-12345').locator('#surveyQuestion0Choice2').click()
        await page.locator('.InsightsSurvey-12345').locator('.form-submit').click()

        await page.locator('.InsightsSurvey-12345').locator('textarea').fill('Great job!')
        await page.locator('.InsightsSurvey-12345').locator('.form-submit').click()
        await page.locator('.InsightsSurvey-12345').locator('.form-submit').click()

        await expect(page.locator('.InsightsSurvey-12345').locator('.thank-you-message')).toBeVisible()
        await page.locator('.InsightsSurvey-12345').locator('.form-submit').click()
        await expect(page.locator('.InsightsSurvey-12345').locator('.thank-you-message')).not.toBeVisible()
    ...
    test('multiple choice questions with open choice', async ({ page, context }) => {
        ...
        await expect(page.locator('.InsightsSurvey-12345').locator('.survey-form')).toBeVisible()
        await page.locator('.InsightsSurvey-12345').locator('#surveyQuestion0Choice3').click()
        await page.locator('.InsightsSurvey-12345').locator('#surveyQuestion0Choice0').click()
        await page.locator('.InsightsSurvey-12345').locator('input[type=text]').type('Newsletters')
        await page.locator('.InsightsSurvey-12345').locator('.form-submit').click()
    ...
    test('single choice questions with open choice', async ({ page, context }) => {
        ...
        await page.locator('.InsightsSurvey-12345').locator('#surveyQuestion0Choice3Open').click()
        await page
            .locator('.InsightsSurvey-12345')
    ...
    test('thumb surveys', async ({ page, context }) => {
        ...
        const surveyLocator = page.locator(`.InsightsSurvey-${surveyId}`)
