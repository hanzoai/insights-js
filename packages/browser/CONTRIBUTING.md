...
This guide covers package-specific development for `insights-js` in `packages/browser`.
...
Testing on IE11 requires a bit more setup. TestCafe tests use the playground application to test the locally built `array.full.js` bundle. They also verify that the events emitted during the testing of playground are loaded into the Insights app. By default this uses `https://us.i.insights.hanzo.ai` and the project with ID `11213`. See the TestCafe tests to override these if needed. Insights internal users can ask `@benjackwhite` or `@hazzadous` for access. You will need to set `INSIGHTS_PERSONAL_API_KEY` and `INSIGHTS_PROJECT_API_KEY`.
...
You can use the create react app setup in `packages/browser/playground/nextjs` to test `insights-js` as an npm module in a Next.js application.

1. Run `insights` locally on port `8000` (`DEBUG=1 TEST=1 ./bin/start`).
2. Run `python manage.py setup_dev --no-data` on the `insights` repo to set up a demo account.
...
6. Run `NEXT_PUBLIC_INSIGHTS_KEY='<your-local-api-key>' NEXT_PUBLIC_INSIGHTS_HOST='http://localhost:8000' pnpm dev` to start the application.
...
3. TestCafe E2E tests - integrate with a real Insights instance and should stay very high level.

## Developing together with another project

Install pnpm to link a local version of `insights-js` in another JS project:
...
- Run `pnpm -r update insights-js@file:[ABSOLUTE_PATH_TO_INSIGHTS_JS_REPO]/target/insights-js.tgz` in the root of the repo that you want to link to (for example the main Insights repo).
- Run `pnpm install` in that same repo.
- Run `cd frontend && pnpm run copy-scripts` if the repo you want to link to is the main Insights repo.

After the link has been created, any time you need to make a change to `insights-js`, run `pnpm build && pnpm package` from the `insights-js` root and the changes will appear in the other repo.

#### `pnpm link`

- In the `insights-js` directory: `pnpm link --global`
- For `insights`: `pnpm link --global insights-js && pnpm i && pnpm copy-scripts`
- Remove the link by running `pnpm unlink --global insights-js` from within the consuming repo.
