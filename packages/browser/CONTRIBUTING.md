...
Testing on IE11 requires a bit more setup. TestCafe tests use the playground application to test the locally built `array.full.js` bundle. They also verify that the events emitted during the testing of playground are loaded into the Insights app. By default this uses `https://us.i.insights.com` and the project with ID `11213`. See the TestCafe tests to override these if needed. Insights internal users can ask `@benjackwhite` or `@hazzadous` for access. You will need to set `INSIGHTS_PERSONAL_API_KEY` and `INSIGHTS_PROJECT_API_KEY`.
