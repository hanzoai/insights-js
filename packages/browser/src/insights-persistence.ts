...
import { PersistentStore, InsightsConfig, Properties } from './types'
import {
...
const CASE_INSENSITIVE_PERSISTENCE_TYPES: readonly Lowercase<InsightsConfig['persistence']>[] = [
    'cookie',
    'localstorage',
    'localstorage+cookie',
    'sessionstorage',
...
const parseName = (config: InsightsConfig): string => {
    let token = ''
    if (config['token']) {
...
/**
 * Insights Persistence Object
 * @constructor
 */
export class InsightsPersistence {
    private _config: InsightsConfig
    props: Properties
    ...
    /**
     * @param {InsightsConfig} config initial Insights configuration
     * @param {boolean=} isDisabled should persistence be disabled (e.g. because of consent management)
     */
    constructor(config: InsightsConfig, isDisabled?: boolean) {
        this._config = config
        this.props = {}
    ...
    private _buildStorage(config: InsightsConfig) {
        if (
            CASE_INSENSITIVE_PERSISTENCE_TYPES.indexOf(
                config['persistence'].toLowerCase() as Lowercase<InsightsConfig['persistence']>
            ) === -1
        ) {
            logger.critical(
        ...
        const storage_type = config['persistence'].toLowerCase() as Lowercase<InsightsConfig['persistence']>
    ...
    update_config(config: InsightsConfig, oldConfig: InsightsConfig, isDisabled?: boolean): void {
