...
export class InsightsConversations {
    ...
    loadIfEnabled() {
        ...
        // The toolbar's internal Insights instance must not own the conversations
        ...
        try {
            ...
            if (!loadExternalDependency) {
                this._handleLoadError('Insights loadExternalDependency extension not found.')
                return
            }
    ...
    private _completeInitialization(
        initConversationsFn: (config: ConversationsRemoteConfig, insights: Insights) => LazyLoadedConversationsInterface
    ): void {
        ...
        try {
            // Pass config and Insights instance to the extension
     ...
     * const response = await insights.conversations.sendMessage('Hello!', {
     *   name: 'John Doe',
     ...
     * const newConvo = await insights.conversations.sendMessage('Start fresh', undefined, true)
     */
    async sendMessage(
        message: string,
     ...
     * const messages = await insights.conversations.getMessages()
     *
     * // Get messages for specific ticket
     * const messages = await insights.conversations.getMessages('ticket-uuid')
     */
     ...
     *
     * @example
     * await insights.conversations.markAsRead()
     */
     ...
     *
     * @example
     * const tickets = await insights.conversations.getTickets({
     *   limit: 10,
     *   offset: 0,
     *   status: 'open'
     * })
     ...
     *
     * @example
     * const ticketId = insights.conversations.getCurrentTicketId()
     * if (ticketId) {
     ...
     *
     * @example
     * const sessionId = insights.conversations.getWidgetSessionId()
     * if (!sessionId) {
     *   // Conversations not available yet
     *   insights.conversations.show()
     * }
     */
