...
export class InsightsSurveys {
    ...
    private _completeSurveyInitialization(
        generateSurveysFn: (instance: Insights, isSurveysEnabled: boolean) => any,
     ...
     * ### Usage:
     *
     *     insights.onSurveysLoaded((surveys) => {
     *         // You can work with all surveys
     *         console.log('All available surveys:', surveys)
     *
     *         // Or get active matching surveys
     *         insights.getActiveMatchingSurveys((activeMatchingSurveys) => {
     *             if (activeMatchingSurveys.length > 0) {
     *                 insights.renderSurvey(activeMatchingSurveys[0].id, '#survey-container')
     *             }
     *         })
     *     })
     *
