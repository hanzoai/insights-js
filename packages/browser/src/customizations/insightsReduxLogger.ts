import { BucketedRateLimiter, isEmptyObject, isNullish, isObject, isUndefined, Logger } from '@hanzo/insights-core'
...
export type InsightsStateLogger = (title: string, stateEvent: StateEvent) => void

export interface InsightsStateLoggerConfig<S = any> {
     ...
     * normally this is only changed with insights support assistance
     */
...
// we need a insights logger for the rate limiter
const phConsoleLogger: Logger = createLogger('[Insights Action RateLimiting]')

export function browserConsoleLogger(title: string, stateEvent: StateEvent): void {
    // but the insights logger swallows messages unless debug is on
...
/**
 * Logger that sends state events to Insights session recordings
 * Requires that the loaded insights instance is provided
 * And returns the function to use as the logger
 *
 * e.g. const config = { logger: sessionRecordingLoggerForInsightsInstance(insights) }
 */
export const sessionRecordingLoggerForInsightsInstance: (insights: Insights) => InsightsStateLogger =
    (insightsInstance: Insights) =>
    (title: string, stateEvent: StateEvent): void => {
        insightsInstance?.sessionRecording?.tryAddCustomEvent('app-state', { title, stateEvent })
    }
 ...
 */
export function insightsKeaLogger<S = any>(config: InsightsStateLoggerConfig<S> = {}) {
    const middleware = insightsReduxLogger(config)

    return {
        name: 'insights-kea-logger',
        events: {
 ...
 */
export function insightsReduxLogger<S = any>(
    config: InsightsStateLoggerConfig<S> = {}
