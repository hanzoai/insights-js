...
/* insights.init is called with `Partial<InsightsConfig>`
 ...
 * The OnlyValidKeys type ensures that only keys that are valid in the InsightsConfig type are allowed.
 */
...
class DeprecatedWebPerformanceObserver {
    ...
    set _forceAllowLocalhost(value: boolean) {
        logger.error(
            'WebPerformanceObserver is deprecated and has no impact on network capture. Use `_forceAllowLocalhostNetworkCapture` on `insights.sessionRecording`'
        )
...
/**
 *
 * This is the SDK reference for the Insights JavaScript Web SDK.
 ...
 * to integrate Insights into your project.
 *
 * This SDK is designed for browser environments.
 * Use the Insights [Node.js SDK](/docs/libraries/node) for server-side usage.
 *
 * @constructor
 */
export class Insights implements InsightsInterface {
    static __defaultExtensionClasses: InsightsConfig['__extensionClasses'] = {}

    __loaded: boolean
    config: InsightsConfig
    _originalUserConfig?: Partial<InsightsConfig>
    ...
    sessionPersistence?: InsightsPersistence
    ...
    /**
     * Initializes a new instance of the Insights capturing object.
     *
     * @remarks
     ...
     * @public
     *
     * @param token - Your Insights API token
     * @param config - A dictionary of config options to override
     * @param name - The name for the new insights instance that you want created
     *
     * {@label Initialization}
     *
     * @returns The newly initialized Insights instance
     */
    init(
        token: string,
        config?: OnlyValidKeys<Partial<InsightsConfig>, Partial<InsightsConfig>>,
        name?: string
    ): Insights {
        if (!name || name === PRIMARY_INSTANCE_NAME) {
            ...
            const namedInsights = instances[name] ?? new Insights()
            namedInsights._init(token, config, name)
            instances[name] = namedInsights

            // Add as a property to the primary instance (this isn't type-safe but it is how it was always done)
            ;(instances[PRIMARY_INSTANCE_NAME] as any)[name] = namedInsights

            return namedInsights
        }
    }

    // insights._init(token:string, config:object, name:string)
    //
    // This function sets up the current instance of the insights
    ...
    _init(token: string, config: Partial<InsightsConfig> = {}, name?: string): Insights {
        ...
        if (this.__loaded) {
            ...
            console.warn('[Insights.js]', 'You have already initialized Insights! Re-initializing is a no-op')
            return this
        }

        this.__loaded = true
        this.config = {} as InsightsConfig // will be set right below
        ...
        this.persistence = new InsightsPersistence(this.config, persistenceDisabled)
        this.sessionPersistence =
            this.config.persistence === 'sessionStorage' || this.config.persistence === 'memory'
                ? this.persistence
                : new InsightsPersistence({ ...this.config, persistence: 'sessionStorage' }, persistenceDisabled)
    ...
    /**
     * _execute_array() deals with processing any insights function
     * calls that were called before the Insights library were loaded
     * (and are thus stored in an array so they can be called later)
     *
     * Note: we fire off all the insights function calls && user defined
     * functions BEFORE we fire off insights capturing calls. This is so
    ...
    push(item: SnippetArrayItem): void {
        if (_executeArrayDepth > 0 && isArray(item) && isString(item[0])) {
            ...
            const fn = (Insights.prototype as any)[item[0]]
            if (isFunction(fn)) {
     ...
     * ```js
     * // basic event capture
     * insights.capture('cta-button-clicked', {
    ...
    capture(
        ...
        if (!this.__loaded || !this.persistence || !this.sessionPersistence || !this._requestQueue) {
            logger.uninitializedWarning('insights.capture')
            return
        }
        ...
        if (isUndefined(event_name) || !isString(event_name)) {
            logger.error('No event name provided to insights.capture')
            return
        }
        ...
        if (properties?.$current_url && !isString(properties?.$current_url)) {
            logger.error(
                'Invalid `$current_url` property provided to `insights.capture`. Input must be a string. Ignoring provided value.'
            )
        ...
        // This doesn't handle $set_once, because insights-people doesn't either
    ...
    /**
     * This method is used internally to calculate the event properties before sending it to Insights. It can also be
     * used by integrations (e.g. Segment) to enrich events with Insights properties before sending them to Segment,
     * which is required for some Insights products to work correctly. (e.g. to have a correct $session_id property).
     *
     ...
     * insights.register({ plan: 'premium' })
     * ```
     *
     * {@label Capture}
     *
     ...
     * insights.register({ campaign: 'summer_sale' }, 7) // expires in 7 days
     * ```
     *
     * @public
     *
     ...
     * insights.unregister_for_session('current_flow')
     * ```
     *
     * @public
     *
     ...
     * ```js
     * // check boolean flag
     * if (insights.getFeatureFlag('new-feature')) {
     *     // show new feature
     * }
     * ```
     *
     ...
     * const variant = insights.getFeatureFlag('button-color')
     * if (variant === 'red') {
     *     // show red button
     * }
     * ```
     ...
     * @param {Object|String} options (optional) If {send_event: false}, we won't send an $feature_flag_call event to Insights.
     ...
     * @example
     * ```js
     * if(insights.getFeatureFlag('beta-feature') === 'some-value') {
     *      const someValue = insights.getFeatureFlagPayload('beta-feature')
     *      // do something
     * }
     * ```
     *
     ...
     * @example
     * ```js
     * const result = insights.getFeatureFlagResult('my-flag')
     * if (result?.enabled) {
     ...
     * ```js
     * // multivariate flag
     * const result = insights.getFeatureFlagResult('button-color')
     ...
     * if (insights.isFeatureEnabled('feature', { send_event: false })) {
     ...
     *   )) {
     *     insights.updateEarlyAccessFeatureEnrollment(
     *       betaKey,
     *       false
     *     )
     *     setActiveBetas(
     ...
     *
     * useEffect(() => {
     *   insights.getEarlyAccessFeatures((features) => {
    ...
    /**
     * Exposes a set of events that Insights will emit.
     ...
     * @example
     * ```js
     * insights.on('eventCaptured', (event) => {
     *   console.log(event)
     * })
     * ```
     *
     ...
     * insights.on('featureFlagsReloading', () => {
     ...
     * @example
     * ```js
     * insights.onFeatureFlags(function(featureFlags, featureFlagsVariants, { errorsLoading }) {
     *     // do something
     * })
     * ```
     *
     ...
     *
     * Callback parameters:
     * - surveys: Survey[]: An array containing all survey objects fetched from Insights using the getSurveys method
     ...
     * @example
     * ```js
     * insights.onSurveysLoaded((surveys, context) => { // do something })
     * ```
     *
     *
     ...
     *
     * Can be used, for example, to sync the Insights session id with a backend session.
     *
     ...
     * @example
     * ```js
     * insights.onSessionId(function(sessionId, windowId) { // do something })
     * ```
     *
     ...
     * }
     *
     * insights.getSurveys(callback, false)
     * ```
     *
     * @public
     *
     ...
     * @example
     * ```js
     * insights.getActiveMatchingSurveys((surveys) => {
     *      // do something
     * })
     * ```
     *
     ...
     * @example
     * ```js
     * insights.renderSurvey(coolSurveyID, '#survey-container')
     * ```
     *
     ...
     * insights.displaySurvey('survey-id-123')
     * ```
     *
     * @example
     * ```js
     ...
     * @example
     * ```js
     * insights.canRenderSurveyAsync(surveyId).then((result) => {
     *     if (result.visible) {
     ...
     *
     * @remarks
     * By default, Insights assigns each user a randomly generated `distinct_id`. Use this method to
     * replace that ID with your own unique identifier (like a user ID from your database).
     *
     * @example
     * ```js
     * // basic identification
     * insights.identify('user_12345')
     * ```
     *
     * @example
     * ```js
    ...
    identify(new_distinct_id?: string, userPropertiesToSet?: Properties, userPropertiesToSetOnce?: Properties): void {
        if (!this.__loaded || !this.persistence) {
            return logger.uninitializedWarning('insights.identify')
        }
     ...
     * insights.group('organization', 'org_12345', {
     *     name: 'Acme Corp',
     *     plan: 'enterprise'
     * })
     * ```
     ...
     * insights.group('organization', 'org_12345')
     * insights.group('team', 'team_67890')
     * ```
     *
     * @public
     *
    ...
    group(groupType: string, groupKey: string, groupPropertiesToSet?: Properties): void {
        if (!groupType || !groupKey) {
            logger.error('insights.group requires a group type and group key')
            return
        }
     ...
     * insights.setPersonPropertiesForFlags({'property1': 'value', property2: 'value2'}, false)
     * ```
     *
     * @public
     *
     ...
     * insights.setGroupPropertiesForFlags({'organization': { name: 'CYZ', employees: '11' } })
     * ```
     *
     * @example
     * ```js
     * // Set properties without reload
     * insights.setGroupPropertiesForFlags({'organization': { name: 'CYZ', employees: '11' } }, false)
     * ```
     *
     ...
     * insights.reset(true)  // also resets device_id
     * ```
     *
     * @public
     */
    reset(reset_device_id?: boolean): void {
        ...
        this.register(
            {
                $last_insights_reset: new Date().toISOString(),
            },
            1
        )
    }
     ...
     * The distinct ID is used to associate events with users in Insights.
     *
     * {@label Identification}
     *
     * @example
     ...
     * const userId = insights.get_distinct_id()
     ...
     * insights.init('token', {
     *     loaded: (insights) => {
     *         const id = insights.get_distinct_id()
     *         // use the ID
     *     }
     * })
     * ```
     ...
     * ```js
     * // basic usage
     * insights.get_session_replay_url()
     *
     * @example
     * ```js
     * // timestamp
     * insights.get_session_replay_url({ withTimestamp: true })
     * ```
     *
     * @example
     * ```js
     ...
     *
     * @remarks
     * Insights will use this to link two distinct_ids going forward (not retroactively).
     ...
     * insights.alias('user_12345')
     * ```
     *
     * @example
     * ```js
     * // explicit alias with original ID
     * insights.alias('user_12345', 'anonymous_abc123')
     * ```
     *
     * @public
     *
    ...
    alias(alias: string, original?: string): CaptureResult | void | number {
        ...
        // insights.people.identify() call made for this user. It is VERY BAD to make an alias with
    ...
    /**
     * Updates the configuration of the Insights instance.
     *
     * {@label Initialization}
     *
     * @public
     *
     * @param {Partial<InsightsConfig>} config A dictionary of new configuration values to update
     */
    set_config(config: Partial<InsightsConfig>): void {
        ...
        if (isObject(config)) {
            ...
            this.sessionPersistence =
                this.config.persistence === 'sessionStorage' || this.config.persistence === 'memory'
                    ? this.persistence
                    : new InsightsPersistence({ ...this.config, persistence: 'sessionStorage' }, isPersistenceDisabled)
     ...
     * insights.startSessionRecording(true)
     * ```
     *
     * @example
     * ```js
     ...
     * if (insights.sessionRecordingStarted()) {
     *   insights.stopSessionRecording()
     * }
     * ```
     */
     ...
     * insights.startExceptionAutocapture()
     * ```
     *
     * @example
     * ```js
     ...
     * insights.stopExceptionAutocapture()
     * ```
     */
     ...
     *
     * @remarks
     * get_property() can only be called after the Insights library has finished loading.
     * init() has a loaded function available to handle this automatically.
     *
     * @example
     * ```js
     * // grab value for '$user_id' after the insights library has loaded
     * insights.init('<YOUR PROJECT TOKEN>', {
     *     loaded: function(insights) {
     *         user_id = insights.get_property('$user_id');
     *     }
     * });
     * ```
     * @public
     ...
     *
     * @remarks
     * This is based on browser-level `sessionStorage`, NOT the Insights session.
     * getSessionProperty() can only be called after the Insights library has finished loading.
     * init() has a loaded function available to handle this automatically.
     *
     * @example
     * ```js
     ...
     * insights.init('YOUR PROJECT TOKEN', {
     *     loaded: function(insights) {
     *         user_id = insights.getSessionProperty('user_id');
     *     }
     * });
     * ```
     *
    ...
    /**
     * Returns a string representation of the Insights instance.
     *
     * {@label Initialization}
     *
     * @internal
     ...
     * insights.init('token', { internal_or_test_user_hostname: 'localhost' })
     * ```
     *
     * @public
     */
    setInternalOrTestUser(): void {
        if (!this._requirePersonProcessing('insights.setInternalOrTestUser')) {
            return
        }
     ...
     *
     * @remarks
     * Enables event tracking and data persistence (cookies/localStorage) for this Insights instance.
     ...
     *
     * @remarks
     * Disables event tracking and data persistence (cookies/localStorage) for this Insights instance.
     ...
     * @example
     * ```js
     * if (insights.has_opted_in_capturing()) {
     ...
     * @example
     * ```js
     * if (insights.has_opted_out_capturing()) {
     ...
     * @example
     * ```js
     * const consentStatus = insights.get_explicit_consent_status()
    ...
    /**
     * Checks whether the Insights library is currently capturing events.
     *
     ...
     * {@label Privacy}
     *
     * @see {InsightsConfig.cookieless_mode}
     * @see {InsightsConfig.opt_out_persistence_by_default}
     * @see {InsightsConfig.respect_dnt}
     *
    ...
    /**
     * Clear the user's opt in/out status of data capturing and cookies/localstorage for this Insights instance
     *
     * {@label Privacy}
     *
     * @public
     ...
     *
     * @remarks
     * Debug mode logs all Insights calls to the browser console for troubleshooting.
     * Can also be enabled by adding `?__insights_debug=true` to the URL.
     *
     * {@label Initialization}
     *
     * @example
...
safewrapClass(Insights, ['identify'])
...
const add_dom_loaded_handler = function () {
    ...
    function dom_loaded_handler() {
        ...
        each(instances, function (inst: Insights) {
            inst._dom_loaded()
        })
    }
    ...
    // so let's simply log an error stating Insights couldn't be initialized
    // We're checking for `window` to avoid erroring out on a SSR context
    if (window) {
        logger.error("Browser doesn't support `document.addEventListener` so Insights couldn't be initialized")
    }
}

export function init_from_snippet(): void {
    const insightsMain = (instances[PRIMARY_INSTANCE_NAME] = new Insights())

    const snippetInsights = assignableWindow['insights']
    ...
    if (snippetInsights) {
        ...
        each(snippetInsights['_i'], function (item: [token: string, config: Partial<InsightsConfig>, name: string]) {
            if (item && isArray(item)) {
                const instance = insightsMain.init(item[0], item[1], item[2])

                const instanceSnippet = snippetInsights[item[2]] || snippetInsights
    ...
    assignableWindow['insights'] = insightsMain

    add_dom_loaded_handler()
}

export function init_as_module(): Insights {
    const insightsMain = (instances[PRIMARY_INSTANCE_NAME] = new Insights())
