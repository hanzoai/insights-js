...
import { knownUnsafeEditableEvent } from '@hanzo/insights-core'
...
describe('insights core - before send', () => {
    const baseUTCDateTime = new Date(Date.UTC(2020, 0, 1, 0, 0, 0))
    const eventName = '$event'

    const insightsWith = (configOverride: Pick<Partial<InsightsConfig>, 'before_send'>): Insights => {
        const insights = defaultInsights().init('testtoken', configOverride, uuidv7())
        return Object.assign(insights, {
    ...
    it('can reject an event', () => {
        ...
        ;(insights._send_request as jest.Mock).mockClear()

        const capturedData = insights.capture(eventName, {}, {})
    ...
    it('can take an array of fns', () => {
        ...
        const capturedData = [insights.capture(eventName, {}, {}), insights.capture('to reject', {}, {})]
    ...
    it('can sanitize $set event', () => {
        ...
        const capturedData = insights.capture('$set', {}, { $set: { value: 'provided' } })
    ...
    it('warned when making arbitrary event invalid', () => {
        ...
        const capturedData = insights.capture(eventName, { value: 'provided' }, {})
    ...
    it('logs a warning when rejecting an unsafe to edit event', () => {
        ...
        insights.capture(randomUnsafeEditableEvent, {}, {})
