...
describe('cookieless', () => {
    ...
    const setup = async (config: Partial<InsightsConfig> = {}, token: string = uuidv7()) => {
    ...
    describe('always mode', () => {
        it('should not set any cookies', async () => {
            const { insights, beforeSendMock } = await setup({
            ...
            expect(event.properties.distinct_id).toBe('$insights_cookieless')
    ...
    describe('on_reject mode', () => {
        it('should not send any events before opt in, then send non-cookieless events', async () => {
            ...
            insights.capture('eventBeforeOptIn') // will be dropped
            expect(beforeSendMock).toBeCalledTimes(0)
            expect(insights.has_opted_out_capturing()).toEqual(true)

            // Mock surveys to verify they get loaded
            const mockSurveysLoadIfEnabled = jest.spyOn(insights.surveys, 'loadIfEnabled')
            ...
            expect(insights.sessionRecording).toBeTruthy()
        ...
        it('should not send any events before opt out, then send cookieless events', async () => {
            ...
            insights.capture('eventBeforeOptOut') // will be dropped
            expect(beforeSendMock).toBeCalledTimes(0)
            expect(insights.has_opted_out_capturing()).toEqual(true)
            ...
            insights.capture('eventAfterOptOut')
            ...
            expect(event.properties.distinct_id).toEqual('$insights_cookieless')
        ...
        it('should pick up positive cookie consent on startup and start sending non-cookieless events', async () => {
            ...
            const { insights: previousInsights } = await setup(
                {
                    cookieless_mode: 'on_reject',
            ...
            previousInsights.opt_in_capturing()
            const { beforeSendMock, insights } = await setup(
                {
                    cookieless_mode: 'on_reject',
                    consent_persistence_name: persistenceName,
                    persistence_name: persistenceName,
            ...
            insights.capture('eventWithStoredCookieConsentConfirm')
            expect(beforeSendMock).toBeCalledTimes(1)
            const event = beforeSendMock.mock.calls[0][0]
        ...
        it('should pick up negative cookie consent on startup and start sending cookieless events', async () => {
            ...
            previousInsights.opt_out_capturing()
            const { beforeSendMock, insights } = await setup(
                {
                    cookieless_mode: 'on_reject',
                    consent_persistence_name: persistenceName,
        ...
        it('should reset when switching consent mode from opt in to opt out', async () => {
            ...
            expect(beforeSendMock.mock.calls[3][0].event).toBe(eventName)
            expect(beforeSendMock.mock.calls[3][0].properties.test).toBe(undefined)
            expect(beforeSendMock.mock.calls[3][0].properties.distinct_id).toEqual('$insights_cookieless')
        ...
        it('should start in cookieless mode when opt_out_capturing_by_default is set (regression for #2841)', async () => {
            ...
            expect(pageview.properties.distinct_id).toEqual('$insights_cookieless')
        ...
        it('should start the request queue when opting out (cookieless transport regression #3680)', async () => {
            ...
            expect(JSON.parse(mockedFetch.mock.calls[1][1].body).properties.distinct_id).toEqual('$insights_cookieless')
