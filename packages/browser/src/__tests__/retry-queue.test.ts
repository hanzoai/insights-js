...
describe('RetryQueue', () => {
    ...
    beforeEach(() => {
        retryQueue = new RetryQueue(mockInsights as any)
    ...
    const enqueueRequests = () => {
        ...
        expect(mockInsights._send_request).toHaveBeenCalledTimes(4)
        mockInsights._send_request.mockClear()
    }
    ...
    it('processes retry requests', () => {
        ...
        expect(mockInsights._send_request.mock.calls.map(([arg1]) => arg1.url)).toEqual([
            '/e?retry_count=1',
            '/e?retry_count=1',
            '/e?retry_count=1',
            '/e?retry_count=1',
    ...
    it('tries to send requests via beacon on unload', () => {
        ...
        expect(retryQueue.length).toEqual(0)
        expect(mockInsights._send_request).toHaveBeenCalledTimes(4)
        expect(mockInsights._send_request.mock.calls.map(([arg1]) => arg1.transport)).toEqual([
            'sendBeacon',
            'sendBeacon',
            'sendBeacon',
            'sendBeacon',
    ...
    it('enqueues requests when offline and flushes immediately when online again', () => {
        ...
        expect(mockInsights._send_request).toHaveBeenCalledTimes(0)
