...
describe('identify()', () => {
    ...
    beforeEach(() => {
        ...
        instance = Object.assign(insights, {
            register: jest.fn(),
            featureFlags: {
    ...
    describe('invalid id passed', () => {
        it('does not update user', () => {
            ...
            expect(console.error).toHaveBeenCalledWith(
                '[Insights.js]',
                'Unique user id has not been set in insights.identify'
            )
        })

        it('does not update user when distinct ID is $insights_cookieless', () => {
            console.error = jest.fn()

            instance.debug()

            instance.identify('$insights_cookieless')

            expect(beforeSendMock).not.toHaveBeenCalled()
            expect(instance.register).not.toHaveBeenCalled()
            expect(console.error).toHaveBeenCalledWith(
                '[Insights.js]',
                'The string "$insights_cookieless" was set in insights.identify which indicates an error. This ID is only used as a sentinel value.'
            )
        })
    })
