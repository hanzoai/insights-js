...
function makeInsightsConfig(name: string, persistenceMode: string): InsightsConfig {
...
describe('persistence', () => {
    let library: InsightsPersistence
    ...
    describe.each(persistenceModes)('persistence modes: %p', (persistenceMode) => {
        ...
        beforeEach(() => {
            library = new InsightsPersistence(makeInsightsConfig('test', persistenceMode))
            library.clear()
        })
        ...
        it('should register_once', () => {
            ...
            const lib2 = new InsightsPersistence(makeInsightsConfig('test', persistenceMode))
            expect(lib2.props).toEqual({ distinct_id: 'hi', test_prop: 'test_val' })
        })

        it('should save user state', () => {
            const lib = new InsightsPersistence(makeInsightsConfig('bla', persistenceMode))
        ...
        it('should rebuild storage when cookie_persisted_properties changes via update_config', () => {
            ...
            let config = makeInsightsConfig('test', 'localStorage+cookie')
            const lib = new InsightsPersistence(config)
            ...
            const newConfig = {
                ...makeInsightsConfig('test', 'localStorage+cookie'),
        ...
        it('skips $feature/ properties when cache is stale and TTL is configured', () => {
            const config = {
                ...makeInsightsConfig('test', persistenceMode),
    ...
    describe('localStorage+cookie', () => {
        ...
        it(`should additionally store certain values in cookies if localStorage+cookie`, () => {
            ...
            const newLib = new InsightsPersistence(makeInsightsConfig('test', 'localStorage+cookie'))
        ...
        it('should persist custom properties to cookies when using localStorage+cookie', () => {
            ...
            const persistence = insights.persistence as InsightsPersistence

            persistence.register({ [customProp]: 'test_value' })

            // Get the persistence name from the instance
            // @ts-expect-error - _name is private and only accessible within class 'InsightsPersistence'
        ...
        it('should allow swapping between storage methods', () => {
            ...
            newConfig = makeInsightsConfig('test', 'localStorage')
            lib.update_config(newConfig, config)
            config = newConfig
    ...
    describe('insights', () => {
        it('should not store anything in localstorage, or cookies when in sessionStorage mode', () => {
            ...
            insights.register({ distinct_id: 'test', test_prop: 'test_val' })
...
describe('insights instance persistence', () => {
    beforeEach(() => {
    ...
    it('should not write to storage if opt_out_persistence_by_default and opt_out_capturing_by_default is true', () => {
        ...
        // init insights while opting out
        const insights = defaultInsights().init(
            uuidv7(),
            {
        ...
        const createdStore = (insights.persistence as any)._storage
