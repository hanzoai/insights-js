...
describe('person processing', () => {
    ...
    const setup = async (
        ...
        return { token, beforeSendMock, insights }
    }
    ...
    describe('init', () => {
        ...
        it('should read person_profiles from init config', async () => {
            ...
            // assert
            expect(insights.config.person_profiles).toEqual('never')
        })
    ...
    describe('identify', () => {
        it('should fail if process_person is set to never', async () => {
            // arrange
            const { insights, beforeSendMock } = await setup('never')
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(
                'insights.identify was called, but process_person is set to "never". This call will be ignored.'
            )
        ...
        it('should switch events to $person_process=true if process_person is identified_only', async () => {
            // arrange
            const { insights, beforeSendMock } = await setup('identified_only')
        ...
        it('should not change $person_process if process_person is always', async () => {
            // arrange
            const { insights, beforeSendMock } = await setup('always')
        ...
        it('should preserve initial referrer info across a separate session', async () => {
            ...
            // act
            // s1
            insights.capture('event s1')

            // end session
            insights.sessionManager!.resetSessionId()
            insights.sessionPersistence!.clear()
            ...
            insights.capture('event s2 before identify')
            insights.identify(distinctId)
            insights.capture('event s2 after identify')
        ...
        it('should preserve initial referrer info across subdomain', async () => {
            ...
            mockURLGetter.mockReturnValue('https://example1.com/pathname1?utm_source=foo1')
            // arrange
            const { insights: insights1, beforeSendMock: beforeSendMock1 } = await setup(
                'identified_only',
                undefined,
                persistenceName
            )
            ...
            // act
            // subdomain 1
            insights1.register({ testProp: 'foo' })
            insights1.capture('event s1')
            ...
            mockReferrerGetter.mockReturnValue('https://referrer2.com')
            mockURLGetter.mockReturnValue('https://example2.com/pathname2?utm_source=foo2')
            const { insights: insights2, beforeSendMock: beforeSendMock2 } = await setup(
                'identified_only',
                undefined,
                persistenceName
            )

            insights2.capture('event s2 before identify')
            insights2.identify(distinctId)
            insights2.capture('event s2 after identify')
        ...
        it('should be backwards compatible with deprecated INITIAL_REFERRER_INFO and INITIAL_CAMPAIGN_PARAMS way of saving initial person props', async () => {
            ...
            const { insights, beforeSendMock } = await setup('always')
            insights.persistence!.props[INITIAL_REFERRER_INFO] = {
                referrer: 'https://deprecated-referrer.com',
                referring_domain: 'deprecated-referrer.com',
            }
            insights.persistence!.props[INITIAL_CAMPAIGN_PARAMS] = {
    ...
    describe('capture', () => {
        it('should include initial referrer info iff the event has person processing when in identified_only mode', async () => {
            ...
            // act
            insights.capture('custom event before identify')
            insights._requirePersonProcessing('test')
            insights.capture('custom event after identify')
    ...
    describe('group', () => {
        it('should start person processing for identified_only users', async () => {
            ...
            // act
            insights.capture('custom event before group')
            insights.group('groupType', 'groupKey', { prop: 'value' })
            insights.capture('custom event after group')
        ...
        it('should send the $groupidentify event even if person_processing is set to never', async () => {
            ...
            expect(mockLogger.error).toBeCalledTimes(1)
            expect(mockLogger.error).toHaveBeenCalledWith(
                'insights.setGroupPropertiesForFlags was called, but process_person is set to "never". This call will be ignored.'
            )
    ...
    describe('setPersonProperties', () => {
        it("should not send a $set event if process_person is set to 'never'", async () => {
            ...
            // act
            insights.setPersonProperties({ prop: 'value' })

            // assert
            expect(beforeSendMock).toBeCalledTimes(0)
            expect(mockLogger.error).toBeCalledTimes(1)
            expect(mockLogger.error).toHaveBeenCalledWith(
                'insights.setPersonProperties was called, but process_person is set to "never". This call will be ignored.'
            )
        })
        ...
        it('should start person processing for identified_only users', async () => {
            ...
            // act
            insights.capture('custom event before setPersonProperties')
            insights.setPersonProperties({ prop: 'value' })
            insights.capture('custom event after setPersonProperties')
    ...
    describe('alias', () => {
        it('should start person processing for identified_only users', async () => {
            ...
            // act
            insights.capture('custom event before alias')
            insights.alias('alias')
            insights.capture('custom event after alias')
        ...
        it('should not send a $create_alias event if person processing is set to "never"', async () => {
            ...
            expect(mockLogger.error).toBeCalledTimes(1)
            expect(mockLogger.error).toHaveBeenCalledWith(
                'insights.alias was called, but process_person is set to "never". This call will be ignored.'
            )
        })
    })
    ...
    describe('createPersonProfile', () => {
        it('should start person processing for identified_only users', async () => {
            ...
            // act
            insights.capture('custom event before createPersonProfile')
            insights.createPersonProfile()
            insights.capture('custom event after createPersonProfile')
    ...
    describe('reset', () => {
        it('should revert a back to anonymous state in identified_only', async () => {
            ...
            const { insights, beforeSendMock } = await setup('identified_only')
            insights.identify(distinctId)
            insights.capture('custom event before reset')

            // act
            insights.reset()
            insights.capture('custom event after reset')

            // assert
            expect(insights._isIdentified()).toBe(false)
            expect(beforeSendMock.mock.calls.length).toEqual(3)
    ...
    describe('persistence', () => {
        it('should remember that a user set the mode to always on a previous visit', async () => {
            ...
            insights1.capture('custom event 1')
            const { insights: insights2, beforeSendMock: beforeSendMock2 } = await setup(
                'identified_only',
                undefined,
                persistenceName
            ...
            // act
            insights2.capture('custom event 2')
    ...
    describe('flags', () => {
        it('should default the person mode to identified_only when an incomplete flags response is handled', async () => {
            // arrange
            const { insights, beforeSendMock } = await setup(undefined)
            insights.capture('startup page view')
    ...
    describe('property calls deduplication', () => {
        it('should deduplicate identical consecutive calls to setPersonProperties', async () => {
            ...
            insights.setPersonProperties({ email: 'john@example.com' })
            insights.setPersonProperties({ email: 'john@example.com' })
        ...
        it('should not deduplicate when properties are different', async () => {
            ...
            insights.setPersonProperties({ email: 'john@example.com' })
            insights.setPersonProperties({ email: 'john.doe@example.com' })
        ...
        it('should not deduplicate when set_once properties are different', async () => {
            ...
            insights.setPersonProperties({ email: 'john@example.com' }, { first_seen: 'today' })
            insights.setPersonProperties({ email: 'john@example.com' }, { first_seen: 'yesterday' })
        ...
        it('does not deduplicate when properties are in different order but identical', async () => {
            ...
            insights.setPersonProperties({ name: 'John', email: 'john@example.com' })
            insights.setPersonProperties({ email: 'john@example.com', name: 'John' })

            expect(beforeSendMock).toHaveBeenCalledTimes(2)
        })

        it('should log a message when deduping properties', async () => {
            const { insights } = await setup('always')
        ...
        it('should deduplicate when using people.set with identical properties', async () => {
            ...
            insights.people.set({ email: 'john@example.com' })
            insights.people.set({ email: 'john@example.com' })
        ...
        it('should deduplicate when using people.set_once with identical properties', async () => {
            ...
            insights.people.set_once({ first_seen: 'today' })
            insights.people.set_once({ first_seen: 'today' })
        ...
        it('should not deduplicate when mixing set and set_once with same properties', async () => {
            ...
            insights.people.set({ email: 'john@example.com' })
            insights.people.set_once({ email: 'john@example.com' })
        ...
        it('should deduplicate a setPersonProperties call after identify()', async () => {
            ...
            insights.identify('new-id', { email: 'john@example.com' }, { first_seen: 'today' })
            insights.setPersonProperties({ email: 'john@example.com' }, { first_seen: 'today' })
        ...
        it('should not deduplicate a setPersonProperties call after identify() if the $set properties are different', async () => {
            ...
            insights.identify('new-id', { email: 'john@example.com' }, { first_seen: 'today' })
            insights.setPersonProperties({ email: 'jane@example.com' }, { first_seen: 'today' })
