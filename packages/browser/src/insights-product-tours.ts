...
const isProductToursEnabled = (instance: Insights): boolean => {
...
export class InsightsProductTours {
    ...
    private _loadScript(cb: () => void): void {
        if (assignableWindow.__InsightsExtensions__?.generateProductTours) {
            cb()
            return
        }
        assignableWindow.__InsightsExtensions__?.loadExternalDependency?.(this._instance, 'product-tours', (err) => {
            if (err) {
    ...
    private _startProductTours(): void {
        if (this._productTourManager || !assignableWindow.__InsightsExtensions__?.generateProductTours) {
            return
        }
        this._productTourManager = assignableWindow.__InsightsExtensions__.generateProductTours(this._instance, true)
    }
