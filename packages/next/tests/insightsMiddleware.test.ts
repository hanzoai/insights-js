...
import { insightsMiddleware } from '../src/middleware/insightsMiddleware'
...
const COOKIE_NAME = 'ph_phc_test123_insights'

describe('insightsMiddleware', () => {
    ...
    it('returns a middleware function', () => {
        const middleware = insightsMiddleware({ apiKey: 'phc_test123' })
    ...
    describe('apiKey resolution', () => {
        it('reads apiKey from NEXT_PUBLIC_INSIGHTS_KEY when not in config', async () => {
            ...
            const middleware = insightsMiddleware({})
            const req = new MockNextRequest('https://example.com/')

            await middleware(req as any)

            expect(mockCookiesSet).toHaveBeenCalledWith(
                'ph_phc_from_env_insights',
                expect.any(String),
                expect.any(Object)
            )
        })
    ...
    describe('cookie seeding', () => {
        ...
        it('uses custom cookieMaxAgeSeconds when provided', async () => {
            const middleware = insightsMiddleware({ apiKey: 'phc_test123', cookieMaxAgeSeconds: 3600 })
            const req = new MockNextRequest('https://example.com/')
    ...
    describe('proxy', () => {
        it('proxies ingest requests to Insights host', async () => {
            ...
            expect(rewriteUrl.origin).toBe('https://insights.hanzo.ai')
        ...
        it('supports custom host', async () => {
            const middleware = insightsMiddleware({
                apiKey: 'phc_test123',
                proxy: { host: 'https://eu.insights.hanzo.ai' },
            })
            const req = new MockNextRequest('https://example.com/ingest/e/')
            ...
            const rewriteUrl: URL = mockNextResponseRewrite.mock.calls[0][0]
            expect(rewriteUrl.origin).toBe('https://eu.insights.hanzo.ai')
            expect(rewriteUrl.pathname).toBe('/e/')
        })
