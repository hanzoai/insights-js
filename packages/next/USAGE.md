...
NEXT_PUBLIC_INSIGHTS_HOST=https://us.i.insights.hanzo.ai
```
...
```ts
// middleware.ts
import { postHogMiddleware } from '@insights/next'
...
The middleware generates a UUIDv7 anonymous ID and sets the `hi_<key>_insights` cookie on the first request. This ensures both client and server share the same identity from the very first render.
...
```ts
// Simplest — defaults to path prefix '/ingest' and host 'https://us.i.insights.hanzo.ai'
export default postHogMiddleware({ proxy: true })
```
...
interface InsightsProxyOptions {
    ...
    host?: string // default: 'https://us.i.insights.hanzo.ai'
}
```
...
The identity cookie is named `hi_<sanitized_key>_insights` and contains JSON:
