...
export interface InsightsProxyOptions {
    ...
    pathPrefix?: string
    /** Insights ingest host to rewrite to. Default: 'https://insights.hanzo.ai' */
    host?: string
}
...
export interface InsightsMiddlewareOptions {
     ...
     *     return insightsMiddleware({ response })(request)
     * }
     * ```
     */
 ...
 * on first visit and optionally rewrites API requests to Insights'
 * ingest host.
 *
 ...
 * ```
 */
export function insightsMiddleware(config: InsightsMiddlewareOptions = {}) {
    const apiKey = resolveApiKey(config.apiKey)
    const proxyConfig = resolveProxyConfig(config.proxy)

    return async function middleware(request: NextRequest) {
        // Proxy ingest requests to Insights' host. These are API calls
