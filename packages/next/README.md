...
NEXT_PUBLIC_INSIGHTS_HOST=https://us.i.insights.hanzo.ai  # optional
```
...
| --------------------- | --------------- | ---------------------------------------------------------------------------------- |
| `@insights/next`       | Client + Server | `InsightsProvider`, `InsightsPageView`, hooks, `getInsights()`, `postHogMiddleware()` |
...
| `NEXT_PUBLIC_INSIGHTS_HOST` | No                          | Custom Insights host (defaults to `https://us.i.insights.hanzo.ai`) |

## How It Works

1. **Middleware** runs on every request and seeds an identity cookie (`hi_<key>_insights`) with a UUIDv7 anonymous ID if none exists. It optionally proxies SDK API calls through your domain.
