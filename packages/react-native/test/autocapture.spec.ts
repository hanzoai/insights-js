...
describe('Insights React Native', () => {
  ...
  describe('autocapture', () => {
    ...
    it('should capture a valid event', () => {
      const mockInsights = { autocapture: jest.fn() } as any
      autocaptureFromTouchEvent({ _targetInst: goodEvent, nativeEvent }, mockInsights)
      expect(mockInsights.autocapture).toHaveBeenCalledTimes(1)
      expect(mockInsights.autocapture.mock.calls[0]).toMatchSnapshot()
    })

    it('should ignore an invalid event', () => {
      const mockInsights = { autocapture: jest.fn() } as any
      autocaptureFromTouchEvent({ _targetInst: ignoreEvent, nativeEvent }, mockInsights)
      expect(mockInsights.autocapture).toHaveBeenCalledTimes(0)
    })
    ...
    it('should handle animated styles without errors', () => {
      ...
      expect(() => {
        autocaptureFromTouchEvent(eventWithAnimatedStyle, mockInsights)
      }).not.toThrow()

      // Should still capture the event, just with empty style
      expect(mockInsights.autocapture).toHaveBeenCalledTimes(1)
      const capturedElement = mockInsights.autocapture.mock.calls[0][1][0]
    ...
    it('should handle mixed animated and regular styles', () => {
      ...
      autocaptureFromTouchEvent(eventWithMixedStyle, mockInsights)
