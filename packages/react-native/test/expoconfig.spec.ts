import {
  addInsightsWithBundledScriptsToBundleShellScript,
...
// Extracts the argument that would become $1 inside insights-xcode.sh when
// the wrapped line is executed by the shell. The shell runs:
//   /bin/sh <insights-xcode.sh-path> <...rest>
// so $1 is the token immediately after the insights-xcode.sh path.
const extractArg1 = (wrappedLine: string): string => {
  // The line looks like: /bin/sh `<node eval>` <arg1> ...
  // Split on the backtick-delimited insights-xcode.sh path expression, then
  // take the first whitespace-separated token from whatever follows it.
  const afterInsights = wrappedLine.split(/`[^`]+`/)[1] ?? ''
  return afterInsights.trim().split(/\s+/)[0]
}

describe('addInsightsWithBundledScriptsToBundleShellScript', () => {
  it('wraps the react-native-xcode.sh invocation with insights-xcode.sh', () => {
    const original = '"../node_modules/react-native/scripts/react-native-xcode.sh"'
    const wrapped = addInsightsWithBundledScriptsToBundleShellScript(original)
    expect(wrapped).toContain('insights-xcode.sh')
    expect(wrapped).toContain('react-native-xcode.sh')
    expect(wrapped.startsWith('/bin/sh ')).toBe(true)
    expect(wrapped.indexOf('insights-xcode.sh')).toBeLessThan(wrapped.indexOf('react-native-xcode.sh'))
  })
  ...
  // Expo SDK 53+ and plain RN projects), insights-xcode.sh receives /bin/sh as
  ...
  ])('arg1 passed to insights-xcode.sh is react-native-xcode.sh path, not /bin/sh — %s', (_desc, original) => {
    const wrapped = addInsightsWithBundledScriptsToBundleShellScript(original)
...
describe('modifyExistingXcodeBuildScript', () => {
  it('wraps the bundle phase shellScript', () => {
    ...
    expect(parsed).toContain('insights-xcode.sh')
  })
