...
# Check for insights-cli using installer environment variables
...
if [ -f "$HOME/.insights/insights-cli" ]; then
  PH_CLI_PATH="$HOME/.insights/insights-cli"
else
  # Check if installed via npm -g @hanzo/cli
  NPM_GLOBAL_PREFIX=$(npm prefix -g 2>/dev/null)
  if [ -n "$NPM_GLOBAL_PREFIX" ] && [ -f "$NPM_GLOBAL_PREFIX/bin/insights-cli" ]; then
    PH_CLI_PATH="$NPM_GLOBAL_PREFIX/bin/insights-cli"
  else
    # Check if installed as local dependency
    NPM_LOCAL_ROOT=$(npm root 2>/dev/null)
    if [ -n "$NPM_LOCAL_ROOT" ] && [ -f "$NPM_LOCAL_ROOT/.bin/insights-cli" ]; then
      PH_CLI_PATH="$NPM_LOCAL_ROOT/.bin/insights-cli"
    else
      # Fallback to searching common locations
      export PATH="/usr/local/bin:/opt/homebrew/bin:$HOME/.cargo/bin:$HOME/.local/bin:$HOME/.insights:$PATH"
      PH_CLI_PATH=$(command -v insights-cli 2>/dev/null)
    fi
  fi
fi

if [ -z "$PH_CLI_PATH" ] || [ ! -x "$PH_CLI_PATH" ]; then
  echo "error: insights-cli not found"
  exit 1
fi
...
if [[ "$SKIP_BUNDLING" ]]; then
  echo "SKIP_BUNDLING enabled; skipping insights-cli upload calls."
  exit 0;
fi
set -x -e
...
if [ $CLONE_EXIT_CODE -eq 0 ]; then
  ...
  echo "error: insights-cli - $CLI_CLONE_OUTPUT"
...
if [ $UPLOAD_EXIT_CODE -eq 0 ]; then
  ...
  echo "error: insights-cli - $CLI_UPLOAD_OUTPUT"
