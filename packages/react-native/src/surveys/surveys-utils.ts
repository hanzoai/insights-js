...
export function resolveSurveyAlignment(position: string | undefined): {
  ...
  if (position) {
    if (KNOWN_SURVEY_POSITIONS.has(position)) {
      ...
      console.warn(
        `[Insights.surveys] Unknown survey position ${JSON.stringify(position)} — falling back to ${defaultSurveyAppearance.position}. Expected one of: ${Object.values(SurveyPosition).join(', ')}.`
      )
    }
  }
  const isTop =
...
function getRatingBucketForResponseValue(responseValue: number, scale: number): string {
  if (scale === 3) {
    if (responseValue < 1 || responseValue > 3) {
      console.warn('Insights Debug: Rating response out of range for scale 3:', responseValue)
    ...
    if (responseValue < 1 || responseValue > 5) {
      console.warn('Insights Debug: Rating response out of range for scale 5:', responseValue)
      return 'neutral' // Default to neutral for out-of-range values
    }
    ...
    if (responseValue < 1 || responseValue > 7) {
      console.warn('Insights Debug: Rating response out of range for scale 7:', responseValue)
      return 'neutral' // Default to neutral for out-of-range values
    }
    ...
    if (responseValue < 0 || responseValue > 10) {
      console.warn('Insights Debug: Rating response out of range for scale 10:', responseValue)
  ...
  console.warn('Insights Debug: Unsupported rating scale:', scale)
  ...
  if (question.branching.type === SurveyQuestionBranchingType.End) {
    ...
    if (question.type === SurveyQuestionType.SingleChoice) {
      ...
      if (!isInteger(response)) {
        console.warn('Insights Debug: Expected integer response for rating question but received:', response)
