...
export type InsightsSurveyProviderProps = {
  // /**
  ...
  /**
   * The default appearance for surveys when not specified in Insights.
   */
...
export function InsightsSurveyProvider(props: InsightsSurveyProviderProps): JSX.Element {
  const insightsFromHook = useInsights()
  const insights = props.client ?? insightsFromHook
  ...
  const activatedSurveys = useActivatedSurveys(insights, surveys)

  const flags = useFeatureFlags(insights)

  // Load surveys once
  useEffect(() => {
    insights
      .ready()
      .then(() => insights._onSurveysReady())
      .then(() => insights.getSurveys())
      .then(setSurveys)
      .catch(() => {})
  }, [insights])
