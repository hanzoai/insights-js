...
export interface InsightsOptions extends InsightsCoreOptions {
   ...
   */
  sessionReplayConfig?: InsightsSessionReplayConfig
...
export class Insights extends InsightsCore {
  ...
  private _sessionReplayOptions?: InsightsOptions
   ...
   * @public
   *
   * @param apiKey - Your Insights API key
  ...
  constructor(apiKey: string, options?: InsightsOptions) {
    ...
    const initAfterStorage = (): void => {
      ...
      const cachedRemoteConfig = this.getPersistedProperty<Omit<InsightsRemoteConfig, 'surveys'>>(
   ...
   * cached via InsightsPersistedProperty.RemoteConfig and applied at startup in startSessionReplay().
   *
   * @internal
   */
   ...
   */
  register(properties: InsightsEventProperties): Promise<void> {
   ...
   * insights.unregister('icecream pref')
   * ```
   *
   * @public
   *
   ...
   *
   * By default, Insights has tracking enabled unless it is forcefully disabled by default using the option { defaultOptIn: false }.
   ...
   * const isEnabled = insights.isFeatureEnabled('key-for-your-boolean-flag')
   * ```
   *
   * @public
   *
   ...
   * const value = insights.getFeatureFlag('key-for-your-boolean-flag')
   * ```
   *
   * @public
   *
   ...
   * const payload = insights.getFeatureFlagPayload('key-for-your-multivariate-flag')
   * ```
   *
   * @public
   *
   ...
   *
   * Insights loads feature flags when instantiated and refreshes whenever methods are called that affect the flag.
   ...
   * insights.group('company', 'company_id_in_your_db')
   * ```
   *
   * @example
   * ```js
   ...
   */
  group(groupType: string, groupKey: string, properties?: InsightsEventProperties): void {
   ...
   * This returns either the ID automatically generated by Insights or the ID that has been passed by a call to identify().
   *
   * {@label Identification}
   *
   * @example
   ...
   *
   * These properties are automatically attached to the current group (set via insights.group()).
   ...
   * insights.setGroupPropertiesForFlags({'company': {'property1': 'value', property2: 'value2'}})
   * ```
   *
   * @public
   *
   ...
   */
  async screen(name: string, properties?: InsightsEventProperties, options?: InsightsCaptureOptions): Promise<void> {
    await this._initPromise
   ...
   * This method will have no effect if Insights is not enabled, or if session replay is disabled in your project settings.
   *
   * Note: This is only available on iOS and Android. On web/macOS, this is a no-op.
   *
   * Requires `insights-react-native-session-replay` version 1.3.0 or higher.
   *
   ...
   * await insights.startSessionRecording()
   * ```
   *
   * @example
   * ```js
   * // Start a new session recording
   * await insights.startSessionRecording(false)
   * ```
   *
   * @public
   *
  ...
  async startSessionRecording(resumeCurrent: boolean = true): Promise<void> {
    ...
    try {
      ...
      if (!OptionalReactNativeSessionReplay.startRecording) {
        this._logger.warn('startRecording is not available. Please update insights-react-native-session-replay.')
        return
      }
  ...
  async stopSessionRecording(): Promise<void> {
    ...
    try {
      ...
      if (!OptionalReactNativeSessionReplay.stopRecording) {
        this._logger.warn('stopRecording is not available. Please update insights-react-native-session-replay.')
        return
      }
   ...
   * insights.setPersonProperties({ plan: 'premium' }, undefined, false)
   * ```
   *
   * @public
   *
  ...
  public async getSurveys(): Promise<SurveyResponse['surveys']> {
    ...
    const surveys = this.getPersistedProperty<SurveyResponse['surveys']>(InsightsPersistedProperty.Surveys)
  ...
  private _cacheSurveys(surveys: Survey[] | null, source: string): void {
    this.setPersistedProperty<SurveyResponse['surveys']>(InsightsPersistedProperty.Surveys, surveys)
  ...
  private async initializeSessionReplayNative(
    ...
    cachedRemoteConfig?: Omit<InsightsRemoteConfig, 'surveys'>
  ): Promise<boolean> {
    if (!OptionalReactNativeSessionReplay) {
    ...
    const sessionReplay = this.getPersistedProperty(InsightsPersistedProperty.SessionReplay) ?? {}
  ...
  private async captureAppLifecycleEvents(): Promise<void> {
    ...
    const properties: InsightsEventProperties = {}

    if (!isMemoryPersistence) {
      const prevAppBuild = this.getPersistedProperty(InsightsPersistedProperty.InstalledAppBuild) as string | undefined
      const prevAppVersion = this.getPersistedProperty(InsightsPersistedProperty.InstalledAppVersion) as
        | string
        | undefined

      if (!appBuild || !appVersion) {
        this._logger.warn(
          'Insights could not track installation/update/open, as the build and version were not set. ' +
      ...
      this._logger.warn(
        'Insights was initialised with persistence set to "memory", capturing native app events (Application Installed and Application Updated) is not supported.'
      )
    }
  ...
  private async persistAppVersion(): Promise<void> {
    const appBuild = this._appProperties.$app_build
    const appVersion = this._appProperties.$app_version
    this.setPersistedProperty(InsightsPersistedProperty.InstalledAppBuild, appBuild)
    this.setPersistedProperty(InsightsPersistedProperty.InstalledAppVersion, appVersion)
  }
}
