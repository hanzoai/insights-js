...
const DEBUG_ID_PLACE_HOLDER = '__INSIGHTS_CHUNK_ID__'
...
/**
 * Adds Insights Chunk ID polyfill module to the bundle.
 */
export function unstableBeforeAssetSerializationDebugIdPlugin({
  ...
  if (debugIdModuleExists) {
    ...
    console.warn('\n\nChunk ID module found. Skipping Insights Chunk ID module...\n\n')
    return premodules
  }
 ...
 */
export const createInsightsMetroSerializer = (customSerializer?: MetroSerializer): MetroSerializer => {
  ...
  return async function (entryPoint, premodules, graph, options) {
    ...
    if (debugIdModuleExists) {
      // eslint-disable-next-line no-console
      console.warn('Chunk ID module found. Skipping Insights Chunk ID module...')
      return serializer(entryPoint, premodules, graph, options)
    }
