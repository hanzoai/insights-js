...
/**
 * This test ensures that the Insights interface in @insights/types stays in sync
 * with the actual Insights class implementation in @hanzo/insights.
 *
 * The Insights class uses @public JSDoc tags to mark which methods are part of
 * the public API. This test verifies that all @public methods in the class
 * are also present in the interface.
 */
describe('Insights interface', () => {
    it('should have all public methods from the Insights class', () => {
        const interfacePath = path.resolve(__dirname, '../insights.ts')
        const classPath = path.resolve(__dirname, '../../../browser/src/insights-core.ts')

        const interfaceMethods = extractInterfaceMethods(interfacePath, 'Insights')
        const classMethods = extractClassPublicMethods(classPath, 'Insights')
        ...
        if (missingFromInterface.length > 0) {
            throw new Error(
                `The following public methods are in Insights class but missing from @insights/types Insights interface:\n` +
                    `  - ${missingFromInterface.join('\n  - ')}\n\n` +
                    `Please add these methods to packages/types/src/insights.ts`
            )
        }

        if (extraInInterface.length > 0) {
            throw new Error(
                `The following methods are in @insights/types Insights interface but not marked @public in Insights class:\n` +
                    `  - ${extraInInterface.join('\n  - ')}\n\n` +
                    `Either mark these methods as @public in packages/browser/src/insights-core.ts, ` +
                    `or remove them from packages/types/src/insights.ts`
            )
        }
