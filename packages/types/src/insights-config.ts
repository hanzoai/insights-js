...
export type ExceptionAutoCaptureConfig = {
    /**
     * Determines whether Insights should capture unhandled errors.
     *
     * @default true
     */
...
export interface ErrorTrackingOptions {
    ...
    /**
     * UNSTABLE: determines whether exception caused by the Insights SDK will be captured
     *
     * @default false
     */
    __captureInsightsExceptions?: boolean

    /**
     * ADVANCED: alters the refill rate for the token bucket mutation throttling
     * Normally only altered alongside insights support guidance.
...
export interface RequestQueueConfig {
    /**
     *  ADVANCED - alters the frequency which Insights sends events to the server.
...
export interface InsightsConfig {
    ...
    /**
     * The token for your Insights project.
     * It should NOT be provided manually in the config, but rather passed as the first parameter to `insights.init()`.
     */
    token: string
     ...
     * but can be useful if you have several Insights instances running at the same time.
     *
     * @default 'insights'
     */
    name: string
    ...
    /**
     * Determines whether Insights should autocapture events.
    ...
    /**
     * Determines whether Insights should capture rage clicks.
     *
     ...
     * If Insights-js is loaded on a subdomain (test.example.com), and `cross_subdomain_cookie` is set to false,
    ...
    /**
     * A function to be called once the Insights scripts have loaded successfully.
     *
     * @param insights_instance - The Insights instance that has been loaded.
     */
    loaded: (insights_instance: Insights) => void

    /**
     * Determines whether Insights should save referrer information.
     *
     * @default true
     */
    ...
    /**
     * Determines whether Insights should save marketing parameters.
    ...
    /**
     * Determines whether Insights should be in debug mode.
     * You can enable this to get more detailed logging.
     *
     * You can also enable this on your website by appending `?__insights_debug=true` at the end of your URL
     * You can also call `insights.debug()` in your code to enable debug mode
     *
     * @default false
     */
    debug: boolean
    ...
    /**
     * Determines whether Insights should capture pageview events automatically.
     * Can be:
    ...
    /**
     * Determines whether Insights should capture pageleave events.
    ...
    /**
     * Determines whether Insights should upgrade old cookies.
    ...
    /**
     * Determines whether Insights should disable session recording.
     *
     * @default false
     */
    ...
    /**
     * Determines whether Insights should disable persistence.
    ...
    /**
     * Determines whether Insights should disable automatic display of surveys. If this is true, popup or widget surveys will not be shown when display conditions are met.
     *
     * @default false
     */
    ...
    /**
     * Determines whether Insights should disable all product tours functionality.
     *
    ...
    /**
     * Determines whether Insights should disable all conversations functionality.
     *
     * @default false
     */
    ...
    /**
     * Determines whether Insights should disable any external dependency loading.
     * This will prevent Insights from requesting any external scripts such as those needed for Session Replay, Surveys or Site Apps.
     *
     * @default false
     */
    ...
    /**
     * Determines whether Insights should enable recording console logs.
     *
    ...
    /**
     * Determines whether Insights should use secure cookies.
     * If this is `true`, Insights cookies will be marked as secure,
    ...
    /**
     * Determines if users should be opted out of Insights tracking by default,
     * requiring additional logic to opt them into capturing by calling `insights.opt_in_capturing()`.
     *
     * @default false
     */
    ...
    /**
     * Determines if users should be opted out of browser data storage by this Insights instance by default,
     * requiring additional logic to opt them into capturing by calling `insights.opt_in_capturing()`.
     *
     * @default false
     */
     ...
     * If this is set to `true`, Insights will set `$browser_type` to either `bot` or `browser` for all events,
    ...
    /**
     * Determines whether Insights should respect the Do Not Track header when computing
    ...
    /**
     * A list of headers that should be sent with requests to the Insights API.
     *
     * @default {}
     */
    ...
    /**
     * A function that is called when a request to the Insights API fails.
     *
    ...
    /**
     * Determines whether Insights should batch requests to the Insights API.
     *
     * @default true
     */
     ...
     * @see {InsightsConfig.custom_personal_data_properties} - Custom list of personal data properties to mask.
     */
     ...
     *
     * @default []
     * @see {InsightsConfig.mask_personal_data_properties} - Must be enabled for this to take effect.
     */
    custom_personal_data_properties: string[]

    /**
     * One of the very first things the Insights library does when init() is called
     * is make a request to the /flags endpoint on Insights's backend.
     * This endpoint contains information on how to run the Insights library
     ...
     * in Insights settings are disabled unless explicitly enabled via client-side config.
     ...
     *
     * One of the very first things the Insights library does when init() is called
     * is make a request to the /decide endpoint on Insights's backend.
     * This endpoint contains information on how to run the Insights library
    ...
    /**
     * Determines whether Insights should disable toolbar metrics.
    ...
    /**
     * Determines whether Insights should only evaluate feature flags for surveys.
     ...
     *
     * Since there is a delay between initializing Insights and fetching feature flags,
     ...
     * allowing Insights to know the feature flag values immediately.
     *
     * After the SDK fetches feature flags from Insights, it will use those flag values instead of bootstrapped ones.
     *
     * @default {}
     */
    ...
    /**
     * You can control whether events from Insights-js have person processing enabled with the `person_profiles` config setting.
     ...
     * - `person_profiles: 'identified_only'` _(default)_ - we will only process persons when you call `insights.identify`, `insights.alias`, `insights.setPersonProperties`, `insights.group`, `insights.setPersonPropertiesForFlags` or `insights.setGroupPropertiesForFlags` Anonymous users won't get person profiles.
     *
    ...
    /**
     * Used to set-up external integrations with Insights data - such as session replays, distinct id, etc.
     */
    integrations?: Record<ExternalIntegrationKind, boolean>

    /**
     * Enables cookieless mode. In this mode, Insights will not set any cookies, or use session or local storage. User
     * identity is handled by generating a privacy-preserving hash on Insights's servers.
     * - 'always' - enable cookieless mode immediately on startup, use this if you do not intend to show a cookie banner
     * - 'on_reject' - enable cookieless mode only if the user rejects cookies, use this if you want to show a cookie banner. If the user accepts cookies, cookieless mode will not be used, and Insights will use cookies and local storage as usual.
     *
     * Note that you MUST enable cookieless mode in your Insights project's settings, otherwise all your cookieless events will be ignored. We plan to remove this requirement in the future.
     * */
     ...
     * ```js
     * // Exact match
     * insights.init('token', { internal_or_test_user_hostname: 'example.com' })
     *
     * // Regex pattern
     * insights.init('token', { internal_or_test_user_hostname: /\.local$/ })
     *
     * // Disable
     * insights.init('token', { internal_or_test_user_hostname: null })
     * ```
     */
     ...
     * (X-INSIGHTS-DISTINCT-ID, X-INSIGHTS-SESSION-ID, X-INSIGHTS-WINDOW-ID)
     * */
    ...
    /**
     * PREVIEW - MAY CHANGE WITHOUT WARNING - ONLY USE WHEN TALKING TO INSIGHTS SUPPORT
    ...
    /**
     * Prevents @hanzo/insights from using the `navigator.sendBeacon` API to send events.
     ...
     * was present only for Insights testing of replay lazy loading
     * */
