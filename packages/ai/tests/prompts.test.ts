...
describe('Prompts', () => {
  ...
  const createMockInsights = (options: { personalApiKey?: string; projectApiKey?: string; host?: string } = {}) => {
    return {
      ...
      host: options.host ?? 'https://insights.hanzo.ai',
    } as any
  }
  ...
  describe('get()', () => {
    ...
    it('should use custom host from Insights options', async () => {
      ...
      const insights = createMockInsights({ host: 'https://insights.hanzo.ai' })
      const prompts = new Prompts({ insights })
    ...
    it('should URL-encode prompt names with special characters', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://insights.hanzo.ai/api/environments/@current/llm_prompts/name/prompt%20with%20spaces%2Fand%2Fslashes/?token=phc_test_key',
        expect.any(Object)
      )
    })
    ...
    it('should work with direct options (no Insights client)', async () => {
      ...
      expect(result).toBe(mockPromptResponse.prompt)
      expect(mockFetch).toHaveBeenCalledWith(
        'https://insights.hanzo.ai/api/environments/@current/llm_prompts/name/test-prompt/?token=phc_direct_key',
        {
          method: 'GET',
          headers: {
