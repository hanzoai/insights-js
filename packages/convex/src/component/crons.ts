...
// Override via `INSIGHTS_FLAGS_POLLING_INTERVAL_SECONDS`.
...
export function readPollingIntervalSeconds(): number {
  const raw = (env.INSIGHTS_FLAGS_POLLING_INTERVAL_SECONDS ?? '').trim()
  if (!raw) return DEFAULT_INTERVAL_SECONDS
  const parsed = Number(raw)
  if (!Number.isFinite(parsed) || !Number.isInteger(parsed) || parsed <= 0) {
    console.warn(
      `[Insights] INSIGHTS_FLAGS_POLLING_INTERVAL_SECONDS="${raw}" is not a positive integer; ` +
...
// load-time gate on `INSIGHTS_PERSONAL_API_KEY` sees an empty value at deploy-time module
// analysis and silently drops the cron. The handler in `lib.ts` gates at runtime instead.
crons.interval(
  'Refresh Insights feature flag definitions',
