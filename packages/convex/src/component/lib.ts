import { Insights as InsightsEdge } from '@hanzo/insights-node/edge'
...
/**
 * Brand events sent through this component as `insights-convex` rather than `insights-edge` in the
 * `$lib` / `$lib_version` properties — makes them filterable in Insights and lets us attribute
 * issues to the integration vs. raw `insights-node` usage.
 */
class Insights extends InsightsEdge {
  getLibraryId(): string {
    return 'insights-convex'
  }
...
const DEFAULT_HOST = 'https://insights.hanzo.ai'
 ...
 * installing app wires the values via `app.use(insights, { env: { ... } })`, typically
...
function readConfig(): { projectToken: string; host: string; personalApiKey: string } {
  const projectToken = (env.INSIGHTS_PROJECT_TOKEN ?? '').trim()
  const host = (env.INSIGHTS_HOST ?? '').trim() || DEFAULT_HOST
  const personalApiKey = (env.INSIGHTS_PERSONAL_API_KEY ?? '').trim()
  if (!projectToken) {
    // Convex's typed env-var validation should prevent an empty `INSIGHTS_PROJECT_TOKEN` at deploy time,
    ...
    console.warn(
      '[Insights] INSIGHTS_PROJECT_TOKEN is not configured; this event will be dropped. ' +
        'Set it with `npx convex env set INSIGHTS_PROJECT_TOKEN phc_…` and redeploy.'
    )
  }
...
/**
 * Cache Insights clients across action invocations within the same Convex isolate.
 *
 ...
 */
const clientCache = new Map<string, Insights>()

function getClient(projectToken: string, host: string): Insights {
  const key = `${projectToken}|${host}`
  let client = clientCache.get(key)
  if (!client) {
    client = new Insights(projectToken, { host, flushAt: 1, flushInterval: 0 })
...
function parseProperties(json: string | undefined): Record<string, unknown> | undefined {
  ...
  try {
    ...
    console.warn('[Insights] Failed to parse serialized properties, dropping them.', e)
    return undefined
  }
}
