# Insights Cloudflare KV Cache Example

This example demonstrates how to implement a distributed cache for Insights feature flag definitions using Cloudflare KV storage. It showcases a split read/write pattern optimized for edge workers where low-latency flag evaluation is critical.
...
2. The scheduled job fetches fresh flag definitions from Insights and stores them in Cloudflare KV
...
- You want to minimize API calls to Insights
- You can tolerate flag updates being slightly delayed (up to 5 minutes, or your own cron schedule)

## Prerequisites

- Node.js and pnpm installed
- A Insights account with a project API key and personal API key

## Setup

### 1. Build Local Dependencies

From the root of the @hanzo/insights repository:
...
INSIGHTS_PERSONAL_API_KEY=your_insights_personal_api_key
INSIGHTS_HOST=https://us.i.insights.hanzo.ai
```
...
The `userId` will be randomly generated for each request, and `enabled` will depend on your flag configuration in Insights.
